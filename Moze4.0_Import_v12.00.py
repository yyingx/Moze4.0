...
def apply_compound_rules(df, compound_rules, output_cols):
    ...
    for _, r in compound_rules.iterrows():
        ...
        try:
            ...
            # 掩码计算用向量化的 str.contains（C 实现），比逐行 map(lambda) 快很多。
            # merchant_regex 仍单独编译保留，是因为后面 expand() 捕获组阶段需要用到。
            # 规则里的正则可能带捕获组（如 "滴滴-(.+)"，是给 expand() 用的），
            # 这里只取布尔匹配结果，捕获组内容不需要，屏蔽 pandas 多余的提示。
