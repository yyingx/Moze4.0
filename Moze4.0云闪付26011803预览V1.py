# -*- coding: utf-8 -*-
"""
Moze 4.0 智能倒序跨年版 (Smart Rollover)
版本: 2026.01.18_SmartRollover
核心逻辑:
1. 【倒序跨年】: 默认 2026 年。如果发现月份从 1月 变成了 12月，自动判定为跨至 2025 年。
2. 【铁壁防御】: 保持之前的"银行行封杀"、"时间剔除"、"金额锁定"逻辑。
3. 【标准分类】: 严格执行 4 条转账/借贷铁律。
"""
...
# ⚙️ 用户配置
# ==============================================================================
DEFAULT_YEAR = 2026  # 初始年份 (账单最顶部的年份)
MY_NAME = "应翔"
...
class ProjectionCutter:
    def cut_image(self, image_path):
        ...
        for i in range(len(cut_y_points) - 1):
            ...
            slices.append((img[y1:y2, 0:w], y1))
        return slices, img
...
class MozeOCRTool:
    def __init__(self):
        print(f"{BColors.OKGREEN}🚀 Moze 4.0 智能倒序跨年版启动...{BColors.ENDC}")
    ...
    def parse_images(self, file_paths):
        ...
        for f_path in file_paths:
            ...
            last_month = -1 # 用于记录上一次读到的月份
            
            slices, debug_img = self.cutter.cut_image(f_path)
            if debug_img is None: continue
            
            # 这里的 global_current_date 会随着年份逻辑动态变化
            global_current_date = f"{current_processing_year}/01/01"
            
            debug_canvas = debug_img.copy()

            for img_slice, y_offset in slices:
                ...
                for line_items in lines:
                    ...
                    if dm and not has_money_sign and len(full_text) < 15 and not is_bank_row and not is_footer_year:
                        ...
                        # === 🔥 倒序跨年核心逻辑 🔥 ===
                        # 如果上一次是 1月(1)，这次变成了 12月(12)，说明跨年了 (1 -> 12)
                        # 账单是倒序的，所以月份应该越来越小。如果突然变大，就是跨年。
                        if last_month != -1:
                            if m > last_month: # 比如 1 -> 12
                                current_processing_year -= 1
                                print(f"   📉 [年份回退] 检测到跨年，调整为 {current_processing_year} 年")
                        
                        last_month = m
                        global_current_date = f"{current_processing_year}/{m}/{d}"
                        
                        print(f"   📅 [日期切换] -> {global_current_date}")
                        ...
                        # 蓝框
                        for r in line_items:
                            x, y = int(r[0][0][0]), int(r[0][0][1]) + y_offset
                            w, h = int(r[0][2][0]-r[0][0][0]), int(r[0][2][1]-r[0][0][1])
                            cv2.rectangle(debug_canvas, (x,y), (x+w,y+h), (255,0,0), 2)
                        continue 
                    ...
                    amount_val = 0.0
                    if not is_bank_row:
                        for r in reversed(line_items):
                            clean = self.clean_money(r[1])
                            if '.' in clean and len(clean) > 2:
                                try:
                                    val = float(clean)
                                    # 过滤像时间的小数字
                                    if (0 <= val <= 24.59) and not any(s in r[1] for s in ['+', '-', '¥', '元']):
                    ...
                    if (has_amount or has_keyword) and not is_bank_row:
                        if pending_top:
                            # 注意：如果跨日期了，这里的 global_current_date 已经是最新的了
                            self.add_record_with_logic(pending_top, None, global_current_date)
                            pending_top = None
            ...
            debug_path = os.path.join(os.path.dirname(f_path), "debug_rollover.jpg")
            cv2.imencode('.jpg', debug_canvas)[1].tofile(debug_path)
            print(f"      📸 [Debug] 分析图: {debug_path}")
    ...
    def save(self):
        ...
        filename = f"MOZE_SmartRollover_{ts}.csv"
