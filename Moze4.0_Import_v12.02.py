...
# 备注开头子类别合并正则；一次 extract 替代逐子类别扫全列（已按长度降序，长前缀优先）
MEMO_SUBCAT_PATTERN = re.compile(
    rf"^({'|'.join(map(re.escape, VALID_SUBCATS))})(.*)$")
...
def get_user_input():
    """选择文件和起始日期。支持命令行传入文件路径（右键"发送到"）。"""
    # 右键"发送到"或命令行传入文件时，直接使用参数
    cli_files = [f for f in sys.argv[1:]
                 if Path(f).suffix.lower() in ('.csv', '.xlsx') and Path(f).exists()]
    ...
    if cli_files:
        files = cli_files
        logger.info(f"接收文件: {', '.join(Path(f).name for f in files)}")
    else:
        logger.info("请选择账单文件...")
...
def _apply_single_rules(df, merchant_rules, product_rules, output_cols):
    ...
    # 商家(old)/商品列对所有规则都是同一份数据，循环外归一化一次，避免每条规则重复处理整列
    normalized_targets = {
        '商家(old)': normalize_text_series(df['商家(old)']),
        '商品': normalize_text_series(df['商品']),
    }

    for _, r in single_rules.iterrows():
        merchant_pattern = str(r['商家(old)']).strip()
        product_pattern = str(r['商品']).strip()
        # 单条件规则：商家(old) 非空走商家匹配，否则走商品匹配。
        if merchant_pattern:
            condition_col, pattern = '商家(old)', merchant_pattern
        else:
            condition_col, pattern = '商品', product_pattern
        target = normalized_targets[condition_col]
...
def apply_compound_rules(df, compound_rules, output_cols):
    ...
    # str.contains 走向量化匹配；catch_warnings 在循环外只进出一次，
    # 屏蔽 pandas 对含捕获组正则的提示（捕获组留给后面 expand() 用）。
    with warnings.catch_warnings():
        ...
        for _, r in compound_rules.iterrows():
            ...
            use_regex = int(r.get('is_regex', 0)) == 1
            try:
                # merchant_regex 供后面 expand() 捕获组使用；is_regex=0 时为 None。
                merchant_regex = (
                    re.compile(merchant_pattern)
                    if use_regex and merchant_pattern else None
                )
                product_hit = bill_product.str.contains(
                    product_pattern, regex=use_regex, na=False)
                if merchant_pattern and merchant_pattern.lower() != 'nan':
                    merchant_hit = merchant_old.str.contains(
                        merchant_pattern, regex=use_regex, na=False)
            ...
            for c in output_cols:
                ...
                if c in {'描述', '商家'}:
                    ...
                    if has_group_template:
                        # 含捕获组占位符必须 is_regex=1，否则无 merchant_regex 可 expand
...
def parse_memo_subcategory(df, main_col, sub_col):
    ...
    # 标准子类别：一次性提取备注开头命中的子类别，避免逐个子类别扫描整列
    extracted = memo_series.str.extract(MEMO_SUBCAT_PATTERN, expand=True)
    matched_all = extracted[0].notna()
    if not matched_all.any():
        return df

    keywords_series = extracted[0]
    tails_series = extracted[1].fillna('').str.strip()

    for subcat in pd.unique(keywords_series[matched_all]):
        matches = matched_all & keywords_series.eq(subcat)
        if not matches.any():
            continue
        ...
        if subcat in RECEIVABLE_PAYABLE_SUBCATS:
            has_dot = tails_series.str.contains(r'\.', regex=True, na=False)
            mask_dot = matches & has_dot
            mask_no_dot = matches & ~has_dot

            if mask_dot.any():
                dot_content = tails_series[mask_dot]
                split_df = dot_content.str.split('.', n=1, expand=True)
                obj_values = split_df[0].str.strip()
                desc_values = (
                    split_df[1].str.strip()
                    if 1 in split_df.columns
                    else pd.Series('', index=dot_content.index)
                )
                ...
                df.loc[mask_dot, '对象'] = obj_values
                df.loc[mask_dot, '描述'] = desc_values

            if mask_no_dot.any():
                df.loc[mask_no_dot, '对象'] = tails_series[mask_no_dot]
                df.loc[mask_no_dot, '描述'] = ""
        else:
            df.loc[matches, '描述'] = tails_series[matches]
...
def _map_record_types(df, sub_col, main_col):
    ...
    if mask_mapped.any():
        # 一次性把 (记录类型, 主类别, 项目) 元组展开成三列，避免三次 apply(lambda x: x[i])
        expanded = pd.DataFrame(
            mapped_values[mask_mapped].tolist(),
            index=mapped_values[mask_mapped].index,
            columns=['记录类型', main_col, '项目'],
        )
        df.loc[mask_mapped, ['记录类型', main_col, '项目']] = expanded
    return df
...
def filter_exportable_transactions(df):
    ...
    mask_has_special_keyword = memo.str.contains(
