# -*- coding: utf-8 -*-
"""
Moze 4.0 完美收官版 (Final Polish)
版本: 2026.01.18_Final
核心修复:
1. 【时间修正】: 强力修复 OCR 识别的时间格式错误 (如 20.29 -> 20:29:00)。
2. 【字段清洗】: 严格按照要求，将'描述'字段置空，'名称'字段按逻辑置空。
3. 【逻辑保持】: 继承之前的"银行行封杀"和"转账/借贷"核心逻辑。
"""
...
class MozeOCRTool:
    def __init__(self):
        print(f"{BColors.OKGREEN}🚀 Moze 4.0 完美收官版启动...{BColors.ENDC}")
    ...
    def parse_images(self, file_paths):
        ...
        # 宽容的时间正则: 匹配 22:31, 22.31, 22 31
        time_pattern = re.compile(r'(\d{1,2})[:：\.\s](\d{2})')

        global_current_date = f"{DEFAULT_YEAR}/01/01"
        ...
        for f_path in file_paths:
            ...
            for img_slice, y_offset in slices:
                ...
                for line_items in lines:
                    ...
                    if dm and not has_money_sign and len(full_text) < 15 and not is_bank_row:
                        m, d = map(int, dm.groups())
                        global_current_date = f"{DEFAULT_YEAR}/{m}/{d}"
    ...
    def add_record_with_logic(self, top, bottom, date_str):
        ...
        row = {
            ...
            '手续费': 0, '折扣': 0, 
            '名称': '',
#             if final_object else target_name,  # 如果是借贷对象，名称置空
            '商家': '', 
