...
def filter_valid_transactions(df):
    """过滤退款、关闭和零金额记录。"""
    return df[(~df['当前状态'].isin(INVALID_STATUSES)) & (df['金额'].abs() > 0.0001)].copy()
...
def filter_importable_transactions(df):
    ...
    普通收入默认不导入，只有备注命中收入关键词或特殊手动关键词时才保留。
    """
    memo = normalize_text_series(df['备注'])
    mask_has_special_keyword = memo.str.contains(ALL_SPECIAL_PATTERN, na=False)
    mask_income_with_keyword = (
        df['收/支'] == '收入') & memo.str.contains(INFER_INCOME_PATTERN, na=False)
    return df[(df['收/支'] == '支出') | mask_income_with_keyword | mask_has_special_keyword].copy()
...
def process_main(df_in, df_rules, main_col, sub_col):
    ...
    df = filter_importable_transactions(df)
