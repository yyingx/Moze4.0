        ...
        for f_path in file_paths:
            ...
            for i, text in enumerate(result_texts):
                ...
                if amount is not None:
                    ...
                    # [D] 清洗描述 (去掉 "转账:", "转给" 等废话)
                    desc_final = description.replace("转账", "").replace(
                        "转给", "").replace("来自", "").replace(":", "").strip()
                    if not desc_final:
                        desc_final = "OCR自动识别"
                    ...
                    print(
                        f"      ✅ [识别成功] {current_date} | {record_type} {amount} | {desc_final}")
