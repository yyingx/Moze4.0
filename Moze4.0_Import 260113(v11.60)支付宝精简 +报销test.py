"""
Moze 导入脚本 v11.60 (Clean Names)
Created on Sun Jan 05 2026
Optimized: Tue Jan 13 2026
Fix: Reverted 'Name' auto-fill for generic categories. Only fills Subcategory, leaving Name empty as requested.
@author: TZY_YX
"""
...
# --- 1. DATA_SOURCE (保留作为基础匹配) ---
DATA_SOURCE = {
    'FRUIT': [
        "水果", "果园", "果蔬", "百果园", "鲜果", "果业", "苹果", "香蕉", "红枣", "大枣", "桃子", 
        "水蜜桃", "樱桃", "黄桃", "香梨", "雪梨", "柑橘", "沃柑", "草莓", "甘蔗", "桔子", "沙糖桔", 
        "葡萄", "西瓜", "柚子", "柠檬", "橙子", "菠萝", "榴莲", "山竹", "蓝莓", "荔枝", "龙眼", 
        "哈密瓜", "椰子", "柿子", "杨梅", "李子", "芒果", "猕猴桃", "火龙果", "百香果", "莲雾", 
        "车厘子", "菠萝蜜", "桑葚", "枇杷", "杨桃", "无花果", "圣女果", "小番茄", "姑娘果",
        "西梅", "青枣", "冬枣", "黑布林", "人参果", "丑八怪", "耙耙柑"
    ],
    'DRINK': [
        "饮料", "可乐", "红牛", "奶茶", "东鹏", "果汁", "椰汁", "酸奶", "咖啡", "拿铁", "乐虎", 
        "AD钙奶", "蜜雪冰城"
    ],
    ...
    'VEGETABLE': [
        "蔬菜", "白菜", "菠菜", "上海青", "油麦菜", "生菜", "娃娃菜", "空心菜", "苋菜", "菜芯",
        "韭菜", "香菜", "芹菜", "荠菜", "芥蓝", "土豆", "胡萝卜", "白萝卜", "青萝卜", "红薯", 
        "紫薯", "山药", "芋头", "莲藕", "藕", "洋葱", "大蒜", "生姜", "竹笋", "芦笋", "茭白",
        "西红柿", "茄子", "黄瓜", "西葫芦", "南瓜", "冬瓜", "苦瓜", "丝瓜", "青椒", "彩椒",
        "尖椒", "秋葵", "玉米", "四季豆", "豇豆", "扁豆", "荷兰豆", "毛豆", "豌豆", "蚕豆", 
        "刀豆", "香菇", "金针菇", "平菇", "杏鲍菇", "口蘑", "木耳", "银耳", "茶树菇", "猴头菇",
        "大葱", "小葱", "蒜苗", "蒜苔"
    ],
    'Snack': ["零食", "切糕", "蛋糕", "面包", "腰果"],
    'BEAN_PRODUCT': [
        "豆腐", "豆皮", "腐竹", "豆干", "豆腐泡", "千张", "豆卷", "腐皮", "油豆皮", "内脂豆腐",
        "老豆腐", "嫩豆腐", "冻豆腐", "响铃卷", "素鸡"
    ],
    'INGREDIENTS': [
        "食材", "馒头", "生水饺", "鲜面条", "干面条", "挂面", "火腿", "腊肠", "榨菜", "甜酒",
        "老干妈", "杂酱", "酱料", "炸酱", "酱豆", "辣椒酱", "白砂糖", "食用盐", "生抽", "老抽", 
        "耗油", "料酒", "胡椒粉", "辣椒粉", "蒸肉粉", "火锅底料", "红豆", "绿豆", "黄豆"
    ],
    'PORK': [
    ...
    'Eggs': ["鸡蛋", "土鸡蛋", "生咸鸭蛋", "皮蛋"],
    'COOKED': [
        "熟食", "水煮花生", "花生米", "蚕豆", "毛豆", "藕夹", "茄盒", "锅包肉", "猪头肉", "卤菜", 
        "凉菜", "烧鸡", "烤鸭", "酱牛肉", "熏鱼", "炸带鱼", "炸鱿鱼", "肉丸", "墨鱼丸", "牛肉丸", 
        "鱼丸", "虾滑", "贡丸", "鱿鱼圈", "糍粑", "熟咸鸭蛋", "熟肉肠", "糯米制品", "铁板鸭"
    ],
    'RICE': ["大米", "五常"],
    'MEAL': [
    ...
    'Parking_fee': ["WF7023"],
    'DAILY_NECESSITIES': [
        "日用", "抽纸", "卷纸", "厨房纸", "垃圾袋", "保鲜袋", "保鲜膜", "洗衣液", "洗洁精", "牙膏",
        "洗发水", "一次性手套", "一次性杯", "棉签"
    ],
...
INGREDIENT_PRIORITY = [
    ...
    ('COOKED', '熟食', '食材'),
    ('Snack', '', '零食'),                # 名称留空
    ('DAILY_NECESSITIES', '', '日常用品'), # 名称留空
    ('Clothing_Shoes_Bags', '', '服饰鞋包'), # 名称留空
    ('Furniture_HomeTextiles', '', '家具家纺'), # 名称留空
    ('SERVER', '节点', '虚拟其他'),
...
def process_heuristics(df, main_col, sub_col):
    ...
    for key, name, sub_c in INGREDIENT_PRIORITY:
        pat = PATTERNS[key]
        mask = search_series.str.contains(pat, regex=True) & (~mask_meal) & main_filter
        if key in ['SEAFOOD', 'PORK', 'POULTRY', 'BEEF_MUTTON', 'VEGETABLE']:
            mask &= (~search_series.str.contains(PATTERNS['COOKED'], regex=True))
    ...
    if mask_found.any():
        ...
        if mask_obj.any(): df.loc[mask_obj, '对象'] = extracted[1].str.strip()
    ...
    if mask_has_content.any():
        ...
        # --- 2. 子类.xxx ---
        mask_is_subcat = heads.isin(valid_subcats_set)
        if mask_is_subcat.any():
            idx = mask_is_subcat
            df.loc[idx, sub_col] = heads.loc[idx].values
        ...
        # --- 普通名称覆盖 ---
        mask_is_general = (~mask_is_subcat) & (~mask_is_reim) & mask_has_content
        if mask_is_general.any():
            idx = mask_is_general
            mask_has_dot = df.loc[idx, '描述'].str.contains(r'\.', regex=True)
            if mask_has_dot.any():
                final_idx = mask_has_dot[mask_has_dot].index
                df.loc[final_idx, '名称'] = heads.loc[final_idx].values
                df.loc[final_idx, '描述'] = tails.loc[final_idx].values
...
def process_main(df, df_rules, main_col, sub_col):
    ...
    if uncat_mask.any():
        ...
        for _, r in regex_rules.iterrows():
            match_indices = to_match[to_match.str.contains(r['商家(old)'], regex=True)].index
            if not match_indices.empty:
                for c, rc in rename_map.items(): df.loc[match_indices, rc] = r[c]
    ...
    if '商户单号' in df.columns:
        mask_tb = (df.get('_source_tag', '') == '#AliPay') & df['商户单号'].astype(str).str.strip().str.startswith('T200P4', na=False)
...
def main():
    print(f"{BColors.BOLD}=== Moze 导入脚本 v11.60 (Clean Names) ==={BColors.ENDC}")
    try:
