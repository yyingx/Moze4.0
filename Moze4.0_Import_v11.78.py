...
def apply_rules(df, df_rules, main_col, sub_col):
    """应用字典规则匹配，支持单列(商家)和复合(商家+商品)两种匹配模式"""
    df['商家(old)'] = df.get('交易对方', pd.NA).astype(str).str.strip()

    # '商品' 列若存在于字典中，仅作过滤条件，不写入输出
    filter_cols = {'商家(old)', 'is_regex', '商品'}
    rename_map = {
        c: f'{c}_rule' for c in df_rules.columns if c not in filter_cols}

    # 拆分：简单规则（无商品过滤）/ 复合规则（有商品过滤）
    has_product_col = '商品' in df_rules.columns
    if has_product_col:
        _prod = df_rules['商品'].astype(str).str.strip()
        compound_mask = _prod.notna() & (_prod != '') & (_prod.str.lower() != 'nan')
        simple_rules = df_rules[~compound_mask]
        compound_rules = df_rules[compound_mask]
    else:
        simple_rules = df_rules
        compound_rules = pd.DataFrame()

    # ── 简单规则：精确匹配（原有逻辑不变）──
    df = pd.merge(
        df,
        simple_rules[simple_rules['is_regex'] == 0].rename(
            columns=rename_map).drop(columns=['is_regex', '商品'], errors='ignore'),
        on='商家(old)', how='left'
    )
    df.reset_index(drop=True, inplace=True)
...
def finalize_records(df, main_col, sub_col):
    ...
    mask_fix_wechatpay = (
        df['支付方式'] == "/") & (df['记录类型'].isin(["收入", "应付款项", "返利回馈"]))
    df.loc[mask_fix_wechatpay, '支付方式'] = "零钱3"

    mask_fix_alipay = (df['支付方式'] == "") & (
        df['记录类型'].isin(["收入", "应付款项", "返利回馈"]))
    df.loc[mask_fix_alipay, '支付方式'] = "Alipay_余额"
...
def process_main(df_in, df_rules, main_col, sub_col):
    ...
    # [v11.77] 收入只保留备注命中收入关键词的记录，避免大量未分类收入混入
