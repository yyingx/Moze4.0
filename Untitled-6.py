...
Moze 4.0 OCR 最终独立版
核心原则：【所见即所得】
1. 单行模式: 识别到一笔金额，只生成一条记录。绝不自动生成双向对冲。
2. 字段清洗: 
   - 转账: 清空 名称/商家/项目/描述/标签/对象
   - 借贷: 清空 名称/商家/项目/描述/标签，保留 对象
3. 时间修复: 强力提取 22:31 或 22.31 格式的时间
4. 格式对齐: 日期 2026/1/15
"""
...
class MozeOCRTool:
    ...
    def parse_images(self, file_paths):
        # 1. 日期正则 (匹配 1.15, 01.15, 1-15)
        date_pattern = re.compile(r'^(\d{1,2})[\.月\-/](\d{1,2})') 
        
        # 2. 时间正则 (增强版: 匹配 22:31, 22.31, 22：31)
        # 必须是 HH:MM 格式，防止匹配到金额
        ...
        for f_path in file_paths:
            ...
            for i, text in enumerate(texts):
                ...
                # [A] 锁定日期
                # 核心校验：月份必须 1-12，日期必须 1-31，否则不是日期 (防止 22:31 误判)
                match_date = False
                ...
                if am and not match_date:
                    ...
                    # 往下看 6 行，寻找时间和账户
                    search_range = texts[i+1 : min(i+7, len(texts))]
                    ...
                    if is_internal:
                        # --- 场景1: 内部转账 (单行) ---
                        # 用户要求: 转账时，名称/商家...对象都是空
                        print(f"   🔄 内部转账: {source_account} ({'转出' if is_out else '转入'})")
                        row['主类别'] = '转账'; row['子类别'] = '转账'
                        
                        if is_out:
                            row['记录类型'] = '转出'; row['金额'] = -abs(amount_val)
                        else:
                            row['记录类型'] = '转入'; row['金额'] = abs(amount_val)
