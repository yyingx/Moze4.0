...
class MozeOCRTool:
    ...
    def parse_images(self, file_paths):
        ...
        time_pattern = re.compile(
            r'(?:(\d{1,2})\s*[:：\.]\s*(\d{2}))|^(\d{2})(\d{2})$')
        amount_pattern = re.compile(
            r'([+\-一_半¥Y￥]*)\s*(\d{1,3}(?:,\d{3})*\.\d{2})')
        ...
        for f_path in file_paths:
            ...
            try:
                texts = self.reader.readtext(img_data, detail=0)
            except Exception as e:
                print(f"❌ 读取失败: {e}")
                continue
            ...
            for i, text in enumerate(texts):
                ...
                if tm_check and not date_pattern.match(text) and not amount_pattern.search(text) and len(text) < 10:
                    h, m_val = 0, 0
                    if tm_check.group(1):
                        h, m_val = int(tm_check.group(1)), int(
                            tm_check.group(2))
                    elif tm_check.group(3) and text.isdigit():
                        h, m_val = int(tm_check.group(3)), int(
                            tm_check.group(4))
                ...
                if len(text) < 20 and re.match(r'^\d{1,2}[\.月\-/]\d{1,2}', text):
                    ...
                    if dm:
                        ...
                        if 1 <= m <= 12 and 1 <= d <= 31:
                            current_date_str = self.format_date(
                                DEFAULT_YEAR, m, d)
                ...
                if am and not match_date:
                    ...
                    is_out = False
                    if "转给" in context or "付款" in context:
                        is_out = True
                    elif "来自" in context or "收到" in context:
                        is_out = False
                    else:
                        raw_prefix = am.group(1)
                        is_out = (
                            '-' in raw_prefix or '一半' in raw_prefix or '一' in raw_prefix)

                    # 提取名字
                    clean_name = context.replace("转账", "").replace("转给", "").replace(
                        "来自", "").replace(":", "").replace("付款", "").replace("收到", "").strip()
                    ...
                    # 找账户 & 时间
                    raw_account = ""
                    final_time = buffer_time if buffer_time else "00:00:00"
                    buffer_time = None
                    ...
                    for line in search_range:
                        ...
                        if final_time == "00:00:00":
                            ...
                            if tm:
                                h, m_val = 0, 0
                                if tm.group(1):
                                    h, m_val = int(tm.group(1)), int(
                                        tm.group(2))
                                elif tm.group(3) and line.isdigit():
                                    h, m_val = int(tm.group(3)), int(
                                        tm.group(4))
                                if 0 <= h <= 23 and 0 <= m_val <= 59:
    ...
    def save(self):
        if not self.rows:
            print(f"\n{BColors.WARNING}⚠️ 无数据{BColors.ENDC}")
            return
        ...
        if not os.path.exists(EXPORT_DIR):
            try:
                ...
                pass
        full_path = os.path.join(EXPORT_DIR, filename) if os.path.exists(
            EXPORT_DIR) else filename
        try:
