...
DATA_SOURCE = {
    ...
    'COOKED': [
        ...
        "烧鸡", "烤鸭", "酱牛肉", "熏鱼", "炸带鱼", "炸鱿鱼", "肉丸", "墨鱼丸", "牛肉丸", "鱼丸", "虾滑", "贡丸"
    ],
    'RICE': ["大米", "五常"],
    'MEAL': [
...
def process_heuristics(df, main_col, sub_col):
    ...
    # 6. 大类识别 (顺序: 蔬菜->大米->豆制品->熟食)
    # 熟食放最后，确保覆盖。且所有食材类都要被正餐锁住。
...
def process_main(df, df_rules, main_col, sub_col):
    ...
    if '备注' in df.columns:
        ...
        # [修复] 仅在 BAD_SHOP 且备注有效时覆盖。其他情况一律拼接。
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v10.67 (终极拼接修复版) ==={bcolors.ENDC}")
    try:
