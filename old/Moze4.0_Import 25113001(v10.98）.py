...
# --- 全局变量 (运行时加载) ---
DATA_SOURCE = {}  # 关键词字典 (从 Excel 加载)
PATTERNS = {}     # 正则表达式字典 (动态构建)

# --- [配置区 3] 食材分类优先级 (全局配置) ---
# 格式: (KEY, 显示名称, 子类别)
# 注意: KEY 必须与 Excel 'Keywords' 表中的 Key 一致
INGREDIENT_PRIORITY = [
    ('VEGETABLE', '蔬菜', '食材'), 
    ('RICE', '大米', '食材'), 
    ('BEAN_PRODUCT', '豆制品', '食材'), 
...
def load_keywords_from_excel(rule_path: Path):
    """
    从 Excel 的 'Keywords' 表加载关键词，填充到 DATA_SOURCE
    结构要求: 第1列为Key (如 FRUIT), 第2列为关键词字符串 (如 苹果, 香蕉)
    """
    global DATA_SOURCE
    if not rule_path.exists():
        print(f"{bcolors.FAIL}未找到规则文件: {rule_path}，关键词加载失败！{bcolors.ENDC}")
        return

    print("正在从 Excel 加载关键词...", flush=True)
    try:
        # [优化] 自动探测 Sheet 名称 (兼容 Keywords, KeyWords, keywords 等大小写)
        ...
        if not sheet_target:
            print(f"{bcolors.WARNING}警告: Excel 中未找到 'Keywords' 表 (不区分大小写)，关键词将为空！{bcolors.ENDC}")
            return

        print(f"  读取工作表: {sheet_target}")
        df = pd.read_excel(rule_path, sheet_name=sheet_target, engine='openpyxl')
        
        # 移除空行
        df = df.dropna(subset=[df.columns[0]])
        
        count = 0
        DATA_SOURCE.clear() # 清空旧数据
        
        for _, row in df.iterrows():
            # [Fix] 使用 .iloc 消除 FutureWarning
            # 第1列作为 Key
            key = str(row.iloc[0]).strip()
            # 第2列作为 Words
            words_raw = str(row.iloc[1]) if len(row) > 1 else ""
            
            # 智能分割：支持 中文逗号、英文逗号、空格、顿号
            words_list = re.split(r'[，,、\s]+', words_raw)
            # 去除空字符串
            words_list = [w.strip() for w in words_list if w.strip()]
            
            if words_list:
                DATA_SOURCE[key] = words_list
                count += 1
                
        print(f"  成功加载 {count} 个分类的关键词。")
        
    except ValueError:
        print(f"{bcolors.WARNING}警告: 读取 Keywords 表失败。{bcolors.ENDC}")
    except Exception as e:
        print(f"{bcolors.FAIL}加载关键词出错: {e}{bcolors.ENDC}")

def build_regex_patterns():
    """根据加载的 DATA_SOURCE 构建正则表达式 PATTERNS"""
    global PATTERNS
    PATTERNS.clear()
    ...
    for key, words in DATA_SOURCE.items():
        # 过滤并转义，防止正则报错
        valid_words = [re.escape(w) for w in words if w]
        if valid_words:
            PATTERNS[key] = r"(?:" + "|".join(valid_words) + ")"
...
def load_rules(rule_path: Path):
    ...
    try:
        ...
        for s in xl.sheet_names:
            ...
            if s.lower() == 'keywords': continue # 跳过关键词表
...
def process_heuristics(df, main_col, sub_col):
    ...
    # [双重保险] 强制再检查一遍熟食
    mask_cooked = check_keywords('COOKED')
    mask_cooked = mask_cooked & (~mask_is_meal) & (uncat | (df[main_col].isin(['购物', '居家', '饮食'])))
    if mask_cooked.any():
        df.loc[mask_cooked, sub_col] = '食材'
        df.loc[mask_cooked, '名称'] = '熟食'
        df[sub_col] = df[sub_col].replace("nan", "")
...
def process_main(df, df_rules, main_col, sub_col):
    ...
    # 动态构建高价值词库字符串
    HIGH_VALUE_KEYS = ['FRUIT', 'VEGETABLE', 'BEAN_PRODUCT', 'INGREDIENTS', 'COOKED', 'MEAL', 'RICE', 'CHARGING', 'DAILY_NECESSITIES', 'PORK', 'POULTRY', 'EGGS', 'BEEF_MUTTON', 'SEAFOOD']
    # 从 PATTERNS 中获取对应的正则字符串 (必须在 build_regex_patterns 后)
    HIGH_VALUE_KEYWORDS = "|".join([PATTERNS.get(k, '') for k in HIGH_VALUE_KEYS if k in PATTERNS])
    ...
    if '备注' in df.columns:
        ...
        # 1. 覆盖：垃圾店铺 -> 用备注
        mask_override = is_bad_shop & has_valid_memo
        df.loc[mask_override, '描述'] = raw_memo[mask_override]
        
        # 2. 拼接：正常店铺 -> 拼接
        mask_append = (~is_bad_shop) & has_valid_memo
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v10.98 (Excel 字典驱动版) ==={bcolors.ENDC}")
    try:
        load_settings(RULE_BOOK_PATH)
        # 1. 先加载关键词
        load_keywords_from_excel(RULE_BOOK_PATH)
        # 2. 构建正则
