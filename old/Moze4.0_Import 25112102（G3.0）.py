...
def process_heuristics(df, main_col, sub_col):
    ...
    if mask.any(): df.loc[mask, ['名称', sub_col, main_col, '项目']] = ['纯净水', '纯净水', '饮食', '食']

    # 7. 馒头 -> 食材
    mask = check_keywords("馒头") & uncat
    if mask.any():
        df.loc[mask, ['名称', sub_col, main_col]] = ['馒头', '食材', '饮食']
        df[sub_col] = df[sub_col].replace("nan", "") # 刷新
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v5.9 (果园修复版) ==={bcolors.ENDC}")
    try:
