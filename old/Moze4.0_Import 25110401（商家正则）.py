...
def process_transactions(df_source, df_rules, start_datetime_obj):
    ...
    # ****************** (这是修复) ******************
    # 准备规则列名, 强制添加 '_rule' 后缀
    # (确保 '主类别_rule' 等列一定存在, 即使没有列名冲突)
    cols_to_apply_exact = [col for col in df_rules_exact.columns if col not in ['商家(old)', 'is_regex']]
    rename_dict = {col: f'{col}_rule' for col in cols_to_apply_exact}
    df_rules_exact_renamed = df_rules_exact.rename(columns=rename_dict)
    
    df_target = pd.merge(
        df_target, 
        df_rules_exact_renamed.drop(columns='is_regex'), # 使用重命名后的规则
        on='商家(old)', 
        how='left'
        # 'suffixes' 不再需要, 因为我们已经手动处理了
    )
    # ****************** (修复结束) ******************
    
    # --- 步骤 2: (灵活) 正则匹配 (只对未匹配的行) ---
    print(f"... 2/2: 正在为剩余行执行 {len(df_rules_regex)} 条正则匹配...", flush=True)
    
    # '主类别_rule' 是 merge 带来的列。如果它为空(NaN)，说明精确匹配失败
    # (这一行现在可以安全执行了)
