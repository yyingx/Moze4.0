...
PATTERNS = {
    # 水果
    'FRUIT': r"(?:水果|果园|果蔬|百果园|鲜果|果业|苹果|香蕉|红枣|大枣|桃子|水蜜桃|樱桃|黄桃|香梨|雪梨|柑橘|沃柑|草莓|甘蔗|桔子|沙糖桔|葡萄|哈密瓜|西瓜|甜瓜|柚子|柠檬|橙子|菠萝|凤梨|榴莲|山竹|蓝莓|荔枝|龙眼|椰子|柿子|杨梅|李子|芒果|猕猴桃|火龙果|百香果|莲雾|车厘子)",
    # 饮料
    'DRINK': r"(?:可乐|红牛|奶茶|东鹏|饮料|汽水|果汁|椰汁|酸奶)",
    # 纯净水
    'WATER': r"(?:纯净水|矿泉水)",
    # 餐饮时段
    'MEAL': r"(?:三镇民生|兰州|丝路|沙县|永和四喜|水煎包|老乡鸡|混沌|馄饨|牛肉汤|热干面|黄蜀郎|鸡公煲|小吃|烧烤|食堂|路边摊|麦香园|长沙臭豆腐|东苑一层|东苑二层|西区食堂|包子|小笼包|外勤|板面|水饺|猪脚饭|肠粉|卤肉饭|油泼面|重庆小面|牛杂粉|麻辣香锅|麻辣烫|煎包|煎饼)",
    # 垃圾描述清理
    'CLEAN': r"(?:支付|商户|二维码收款|收款|美团|编号|转账|错误|订单编号|\d{10,})"
}

# --- [配置区 4] 全自动推导引擎 (子类别 -> 主类别, 项目) ---
AUTO_MAP_DICT = {
    # --- 个人 ---
...
def _create_transfer_pair(df_subset, out_acc, in_acc):
    ...
    in_df['金额'] = pd.to_numeric(in_df['金额(元)'])
    in_df['记录类型'] = '转入'
    in_df['账户'] = in_acc
...
def process_transfers(df, main_col, sub_col):
    ...
    for p in icbc_pairs: p[sub_col] = '提现' # 特殊标记
...
def process_heuristics(df, main_col, sub_col):
    ...
    if mask.any():
        ...
        # 仅未分类时填充
        fill_mask = mask & uncat
        if fill_mask.any(): df.loc[fill_mask, ['名称', main_col, sub_col]] = ['充电', '交通', '加油充电']

    # 2. 软件
    mask = check_keywords("软件") & uncat
    if mask.any(): df.loc[mask, [sub_col, main_col]] = ['Software', '虚拟']
    
    # 3. 水果
    mask = check_keywords(PATTERNS['FRUIT']) & (~mask) & allow_overwrite
    if mask.any(): df.loc[mask, ['名称', sub_col, main_col]] = ['水果', '饮料水果', '饮食']
    
    # 4. 饮料
    mask = check_keywords(PATTERNS['DRINK']) & (~mask) & allow_overwrite
    if mask.any(): df.loc[mask, ['名称', sub_col, main_col]] = ['饮料', '饮料水果', '饮食']
    
    # 5. 纯净水
    mask = check_keywords(PATTERNS['WATER']) & (~mask) & allow_overwrite
    if mask.any(): 
        df.loc[mask, [sub_col, main_col]] = ['纯净水', '饮食']
        df.loc[mask, '名称'] = ""

    # 6. 馒头
    mask = check_keywords("馒头") & uncat
    if mask.any():
        df.loc[mask, [sub_col, main_col]] = ['食材', '饮食']
        df.loc[mask, '名称'] = ""
    ...
    # 8. 垃圾信息清理
    mask = df['描述'].str.contains(PATTERNS['CLEAN'], regex=True)
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v10.0 (架构重构版) ==={bcolors.ENDC}")
    try:
