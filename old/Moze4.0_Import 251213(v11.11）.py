...
DATA_SOURCE = {
    ...
    'VEGETABLE': [
        "蔬菜",
        # 叶菜类
        "白菜", "菠菜","上海青", "油麦菜", "生菜", "娃娃菜", "空心菜", "苋菜", "茼蒿", "韭菜", "香菜", "芹菜", "荠菜", "芥蓝",
         # 根茎类
    ...
    'Clothing_Shoes_Bags': ["袜子", "内裤", "帽子", "手套", "鞋", "T恤", "裤", "外套",],
...
def process_heuristics(df, main_col, sub_col):
    ...
    #停车费报销
    mask = check_keywords('Parking_fee') & (~mask) & allow_overwrite
    if mask.any(): 
...
def process_main(df, df_rules, main_col, sub_col):
    ...
    # 步骤 A：如果是淘宝，预填充商品名 (核心改动)
    # 获取原始商品名
    raw_goods = df.get('商品', pd.Series([""] * len(df))).fillna("").astype(str)
    # 仅针对淘宝填充
    mask_is_taobao = (df['商家'] == '淘宝')
    df.loc[mask_is_taobao, '描述'] = raw_goods[mask_is_taobao]
    
    # 步骤 B：如果有备注，优先使用备注 (覆盖上面的淘宝名或空值)
    if '备注' in df.columns:
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v11.11 (淘宝描述特供版) ==={bcolors.ENDC}")
    try:
