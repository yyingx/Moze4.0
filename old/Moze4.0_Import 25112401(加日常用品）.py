...
PATTERNS = {
    ...
    'DRINK': r"(?:可乐|红牛|奶茶|东鹏|饮料|汽水|果汁|椰汁|酸奶|咖啡|拿铁|美式)",
    'WATER': r"(?:纯净水|矿泉水|农夫山泉|怡宝|百岁山|娃哈哈|今麦郎)",
    'SOFTWARE': r"(?:软件)",
    
    # 充电相关
    'CHARGING': r"(?:特来电|星星充电|小桔充电|国家电网|电费|充电|e充电|云快充|蔚来|特斯拉|超充)",
    ...
    # 食材与调料
    'INGREDIENTS': r"(?:馒头|火腿|葱|姜|蒜|酱料|料酒|白砂糖|生抽|老抽|蒸肉粉|榨菜|辣椒酱|老干妈)",
    
    # 熟食
    'COOKED': r"(?:水煮花生|毛豆|藕夹|茄盒|锅包肉|猪头肉|卤菜|凉菜|熟食|烧鸡|烤鸭|酱牛肉|熏鱼)",
    
    # 大米
    'RICE': r"(?:大米)",
    
    # 正餐
    'MEAL': r"(?:三镇民生|兰州|丝路|沙县|永和四喜|水煎包|老乡鸡|混沌|馄饨|牛肉汤|热干面|黄蜀郎|鸡公煲|小吃|烧烤|食堂|路边摊|麦香园|长沙臭豆腐|东苑一层|东苑二层|西区食堂|包子|小笼包|外勤|板面|水饺|猪脚饭|肠粉|卤肉饭|油泼面|重庆小面|牛杂粉|麻辣香锅|麻辣烫|煎包|煎饼|烧饼|锅盔|馕|土豆鸡蛋饼|热干面|牛肉面|刀削面|盖浇饭|炒饭)",
    
    # 日常用品
    'DAILY_NECESSITIES': r"(?:日常用品|锅|勺|刀|砧板|擀面杖|和面棒|打蛋器|压汁器|料理机|计时器|切蛋器|瓶起子|筷|碗|杯|袋|瓶|盒|壶|罐|桶|纸|手套|膜|布|洗洁精|钢丝球|牙刷|牙膏|漱口杯|洗发水|沐浴露|香皂|鼻毛修剪器|掏耳勺|抽纸|卫生纸|湿巾|清凉油|花露水|洗衣粉|洁厕灵|除锈剂|清洗剂|拖把|扫把|水桶|盆|围裙|衣架|晒衣杆|夹子|挂钩|置物架|凳子垫|雨伞|锁|蚊香|香薰|电池|螺丝刀|卷尺|润滑油|胶水|插座|理线器)",

    # 无效店铺名 (用于判断是否被备注覆盖)
    'BAD_SHOP': r"(?:抖音|支付宝|微信|转账|扫码|付款|商户|美团|饿了么|合并支付|京东|拼多多|淘特|特价版)",
    
    # 垃圾字符库 (用于强力清洗)
    'CLEAN': r"(?:2\.00E\+15|经营码交易|余额充值|.*?后付|消费|费用|支付|商户|二维码收款|收款|美团|编号|转账|错误|订单编号|单号|备注|方备注|付款方|XP|\d{10,}|/|[A-Za-z]\d+-\d+)"
}
...
def process_heuristics(df, main_col, sub_col):
    ...
    # 6. 大类识别 (正餐排斥)
    mask_is_meal = check_keywords(PATTERNS['MEAL'])
    ...
    # 7. 日常用品识别
    mask = check_keywords(PATTERNS['DAILY_NECESSITIES'])
...
def process_main(df, df_rules, main_col, sub_col):
    ...
    if '备注' in df.columns:
        ...
        is_bad_shop = df['描述'].astype(str).str.contains(PATTERNS['BAD_SHOP'], regex=True, na=False)
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v10.43 (拼接逻辑修正版) ==={bcolors.ENDC}")
    try:
