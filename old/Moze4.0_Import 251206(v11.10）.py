...
DATA_SOURCE = {
    ...
    # 更新了日用品清单，加入您图片中的长关键词
    'DAILY_NECESSITIES': [
        "日常用品", 
        # 强特征词 (长词)
        "榨汁机", "挤压器", "水果压", "调料罐", "调料盒", "油罐", "盐罐", "料理机", "计时器", "切蛋器", "瓶起子",
        "洗发水", "沐浴露", "鼻毛修剪器", "掏耳勺", "清凉油", "花露水", "卫生纸", "洗洁精", "钢丝球", "洗衣粉", 
        "洁厕灵", "除锈剂", "清洗剂", "螺丝刀", "润滑油", "除锈剂", "理线器", "置物架", "凳子垫",
        # 弱特征词 (单字)
        "锅", "勺", "刀", "砧板", "擀面杖", "和面棒", "打蛋器", "压汁器", "筷", "碗", "杯", 
...
def process_heuristics(df, main_col, sub_col):
    ...
    # 7.1 强特征识别 (榨汁机、调料罐等) -> 强制覆盖食材/水果，但依然保护餐饮
    mask_strong = get_mask(pattern_strong)
    mask_strong = mask_strong & (~mask_is_meal) # 依然防止覆盖'麻辣香锅'
    if mask_strong.any():
        df.loc[mask_strong, [main_col, sub_col]] = ['购物', '日常用品']
        df.loc[mask_strong, '名称'] = ''
        df[sub_col] = df[sub_col].replace("nan", "")

    # 7.2 弱特征识别 (锅、罐、盒等) -> 严格保护，不覆盖任何已识别的食材/餐饮
    mask_weak = get_mask(pattern_weak)
    # 排除已识别的日常用品(避免重复处理)、排除餐饮、排除食材、排除水果
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v11.10 (强弱日用品分级版) ==={bcolors.ENDC}")
    try:
