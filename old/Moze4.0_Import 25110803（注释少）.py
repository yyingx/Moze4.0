...
ALIPAY_MAPPING = {
    '交易时间': '交易时间', '交易分类': '交易类型', '交易对方': '交易对方',
    '商品说明': '商品', '收/支': '收/支', '收/付款方式': '支付方式',
    '交易状态': '当前状态', '交易订单号': '交易单号', '商家订单号': '商户单号',
    '备注': '备注'
}
...
def _process_main_transactions(df_main_raw, df_rules):
    ...
    merchant_mask = df_target['商家(old)'].str.contains(merchant_pattern, na=False, regex=True) | \
                    df_target['商家'].str.contains(merchant_pattern, na=False, regex=True)
    ...
    try:
        ...
        for file_path in source_files:
            ...
            if df_source is not None:
                ...
                print(f"  警告: 文件 {file_path.name} 加载失败，已跳过。", flush=True)
        
        if not list_of_dfs:
            print(f"错误：所有文件均加载失败，程序退出。", flush=True)
            return
        ...
        if uncategorized_count > 0:
            ...
            print(f"  --- 未分类的记录 (最多显示前 10 条) ---", flush=True)
            for row in problem_rows_uncat.head(10).itertuples():
                csv_row_num = row.Index + 2
                print(f"    - [CSV 第 {csv_row_num} 行] [日期: {row.日期}, 商家: {row.商家}, 金额: {row.print_amount}]", flush=True)
            if uncategorized_count > 10:
                print(f"    ... (及其他 {uncategorized_count - 10} 条)", flush=True)
        else:
            print(f"  [通过] 所有记录均已分类 (主类别/子类别 均不为空)。", flush=True)
        ...
        if missing_object_count > 0:
            print(f"  [警告] 发现 {missing_object_count} 条 '应收款项'/'应付款项' 记录缺少 '对象':", flush=True)
            ...
            print(f"  --- 缺少'对象'的记录 (最多显示前 10 条) ---", flush=True)
            for row in problem_rows_obj.head(10).itertuples():
                csv_row_num = row.Index + 2
                print(f"    - [CSV 第 {csv_row_num} 行] [日期: {row.日期}, 描述: {row.描述}, 金额: {row.print_amount}]", flush=True)
            if missing_object_count > 10:
                print(f"    ... (及其他 {missing_object_count - 10} 条)", flush=True)
        else:
            print(f"  [通过] 所有 '应收款项'/'应付款项' 记录均已填写 '对象'。", flush=True)
