...
def process_main(df, df_rules, main_col, sub_col):
    ...
    # 1. 圈定处理范围：子类为空 OR 子类已经是 "正餐"
    mask_allow_refine = (df[sub_col] == "") | (df[sub_col] == "正餐")

    # 2. 识别是否包含餐饮关键词 (商家/商品/描述)
    is_meal_keyword = (
    ...
    # 3. 最终确定要计算时间的行
    mask_final_meal = (is_meal_keyword | (df[sub_col] == "正餐")) & mask_allow_refine
    
    if mask_final_meal.any():
        h = df.loc[mask_final_meal, '交易时间'].dt.hour
        vals = np.select([(h>=6)&(h<11), (h>=11)&(h<16), (h>=16)&(h<21)], ["早餐", "午餐", "晚餐"], default="夜宵")
        df.loc[mask_final_meal, sub_col] = vals
