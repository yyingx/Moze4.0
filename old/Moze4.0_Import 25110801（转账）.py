...
# *** (此版本分离了 "硬编码转账" 和 "主交易" 逻辑) ***
# ******************************************************
def process_transactions(df_source, df_rules, start_datetime_obj):
    """
    核心处理函数：
    1. (已重构) 按业务逻辑分离数据 ("硬编码转账" vs "所有其他")
    2. 封装处理逻辑到辅助函数中
    3. 合并结果
    """
    
    # --- 1. 辅助函数：处理“硬编码转账”（肖恩, 工行提现等） ---
    ...
    def _process_hardcoded_transfers(df_hardcoded_raw):
        ...
        # --- (新增) Logic 2: "工商银行(9579)" 提现 ---
        # (注意) 这里的 '交易对方' 是原始数据中的值
        ...
        if not df_icbc_raw.empty:
            ...
            df_transfer_out['子类别'] = "提现" # (新)

            # B. 创建 "转入" (Transfer In) 行 (按截图)
        ...
        # (关键修复) 使用 .fillna() 来设置默认值，而不是覆盖
        df_all_transfers['主类别'] = "转账" # (共同)
        df_all_transfers['子类别'] = df_all_transfers['子类别'].fillna("转账") # (默认是转账, '提现' 会被保留)
    ...
    def _process_main_transactions(df_main_raw, df_rules):
        ...
        try:
            ...
            account_map_regex = {
                ...
                r'^工商银行储蓄卡\(9579\).*': '工商银行',
                r'^零钱.*': '零钱3'  # (新) 匹配 "零钱", "零钱通" 等 -> "零钱3"
            }
            # (注意) 这里的工商银行(9579) 逻辑仍然保留，
            # 它用于处理 *非提现* 的普通支出 (例如从工行卡支付)
        ...
        # 2E. 描述处理
        df_target = process_descriptions(df_target) # (调用外部辅助函数)
    ...
    # 3B. (已重构) 按业务逻辑分离 (Triage)
    sean_mask = (df_source["交易对方"] == "肖恩")
    icbc_mask = (df_source["交易对方"] == "工商银行(9579)") # (新)
    
    # (新) 包含 肖恩 和 工行提现 的所有记录
    hardcoded_mask = sean_mask | icbc_mask 
    
    df_transfers_raw = df_source[hardcoded_mask].copy() 
    df_main_raw = df_source[~hardcoded_mask].copy() # (包含：其他收入, 其他支出, /)
    ...
    # (关键) 标签逻辑：'转账' 和 '提现' (转入/转出) 不应该有来源标签
