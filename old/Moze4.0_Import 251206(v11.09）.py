...
DATA_SOURCE = {
    ...
    'INGREDIENTS': [
        "馒头", "火腿", "榨菜", "辣椒酱", "老干妈", "杂酱", "生水饺", "酱料", "炸酱", "料酒", "白砂糖", 
        "生抽", "老抽", "蒸肉粉", "胡椒粉", "火锅底料", "辣椒粉", "鲜面条", "干面条", "挂面", "红豆", "绿豆", "黄豆"
    ],
    'PORK': [
    ...
    'DAILY_NECESSITIES': [
        "日常用品", "锅", "勺", "刀", "砧板", "擀面杖", "和面棒", "打蛋器", "压汁器", "料理机", "计时器", "切蛋器", 
        "瓶起子", "筷", "碗", "杯", "牙刷", "牙膏", "漱口杯", "洗发水", "沐浴露", "香皂", "鼻毛修剪器", "掏耳勺", 
        "清凉油", "花露水", "抽纸", "卫生纸", "湿巾", "洗洁精", "钢丝球", "洗衣粉", "洁厕灵", "除锈剂", "清洗剂",
        "拖把", "扫把", "水桶", "盆", "围裙", "螺丝刀", "卷尺", "润滑油", "胶水", "除锈剂", "锁", "灯", "电池", 
        "插座", "理线器", "袋", "瓶", "盒", "壶", "罐", "桶", "纸", "手套", "膜", "布", "衣架", "晒衣杆", 
        "夹子", "挂钩", "置物架", "凳子垫", "雨伞"
    ],
...
def process_heuristics(df, main_col, sub_col):
    ...
    # 【修复】增加对'饮料水果'和'MEAL'(餐饮)的保护
    # 防止'水果罐头'中的'罐'字触发
    # 防止'麻辣香锅'中的'锅'字触发
    mask = mask & (uncat | (df[main_col].isin(['购物', '居家', '饮食']))) 
    mask = mask & (~df[sub_col].isin(['食材', '饮料水果'])) 
    mask = mask & (~mask_is_meal) # 关键修复
...
def process_main(df, df_rules, main_col, sub_col):
    ...
    # ==========================================
    # [核心逻辑] 描述：备注 > 字典 > 空 (不清洗内容)
    # ==========================================
    
    # 基础值：空值
    source_desc = df.get('描述', pd.Series([""] * len(df))).fillna("").astype(str).replace('nan', '')
    
    # 步骤 A：如果有备注，优先使用备注
    if '备注' in df.columns:
        raw_memo = df['备注'].astype(str).replace('nan', '').str.strip()
        df['描述'] = np.where(raw_memo != "", raw_memo, source_desc)
    else:
        df['描述'] = source_desc
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v11.09 (修复餐饮覆盖Bug) ==={bcolors.ENDC}")
    try:
