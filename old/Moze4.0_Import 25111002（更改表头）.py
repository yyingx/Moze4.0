...
# (!!!) 目标抬头 (TARGET_HEADERS) 已被移除
# 它将在 main() 函数中根据您的 Moze Dict.xlsx 动态生成
    ...
    if file_path.suffix == '.csv':
        ...
        try:
            ...
            if cell_A1 == "交易时间":
                ...
                print("  检测到：【合并后的账单】 (快速嗅探)", flush=True)
            elif "微信支付账单明细" in cell_A1: 
                file_type, skip_rows = "WeChat", 16 # 微信表头在第17行，索引为16
...
# (!!!) 签名已更改：增加了 main_cat_col, sub_cat_col
    ...
    if not unmatched_indices.empty and not df_rules_regex.empty:
        ...
        for _, rule in df_rules_regex.iterrows():
            ...
            try:
                ...
                rows_to_update_idx = series_to_match[match_mask].index # 获取原始的 df_target 索引
...
# (!!!) 签名已更改：增加了 target_headers_list, main_cat_col, sub_cat_col
...
def main():
    try:
        ...
        if uncategorized_count > 0:
            ...
            # (!!!) 修正 2 (表头唯一) 确保了 problem_rows_uncat['金额'] 是一个 Series
            # (!!!) 保留健壮的打印逻辑
        ...
        if '记录类型' in df_final.columns and '对象' in df_final.columns:
            ...
            if missing_object_count > 0:
                ...
                for row in problem_rows_obj.head(10).itertuples():
                    csv_row_num = row.Index + 2
                    desc_val = getattr(row, '描述', "N/A") # (!!!) 动态获取描述列
            ...
            print(f"  {bcolors.OKGREEN}[跳过] 未检测到 '记录类型' 或 '对象' 列，跳过 '应收款项' 检查。{bcolors.ENDC}", flush=True)
