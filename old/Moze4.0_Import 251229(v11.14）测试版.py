...
def process_main(df, df_rules, main_col, sub_col):
    ...
    prefix_pat = r"^(" + "|".join(trigger_words) + r")[. 。\s-]+(.*)"
    mask_prefix = df['描述'].str.match(prefix_pat, na=False)
    
    if mask_prefix.any():
        df.loc[mask_prefix, '描述'] = df.loc[mask_prefix, '描述'].str.replace(prefix_pat, r'\2', regex=True).str.strip()
