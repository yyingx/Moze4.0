...
DATA_SOURCE = {
    ...
    'RICE': ["大米", "五常"],
    'MEAL': [
        "正餐", "三镇民生", "兰州", "丝路", "沙县", "永和四喜", "水煎包", "老乡鸡", "混沌", "馄饨", "牛肉汤", "热干面", 
...
def sniff_and_load_data(file_path: Path):
    ...
    try:
        ...
        # 3.1 统一中文括号 (防止 "金额（元）" 匹配不上)
        df.columns = df.columns.astype(str).str.strip().str.replace('（', '(', regex=False).str.replace('）', ')', regex=False)
        
        # 3.2 映射标准列名 (依据脚本开头的 CONFIG 配置)
        df.rename(columns=COLUMN_MAPPING, inplace=True)

        # 3.3 关键列检查 (确保表头行定位正确)
        if '金额(元)' not in df.columns:
            # 这里的 25 和 17 是给人看的行号(索引+1)
            row_hint = "25" if ftype == "AliPay" else "17"
            print(f"  失败: 第 {row_hint} 行看起来不像表头，找不到 '金额(元)' 列。")
            print(f"  当前读取到的列名: {df.columns.tolist()}")
            return None
        
        # 3.4 金额格式清洗 (去货币符号 ¥, 去逗号, 转数字)
...
def process_heuristics(df, main_col, sub_col):
    ...
    # 5. [升级版] 全自动推导 (子类 -> 记录类型 + 主类 + 项目)
    
    mapped_types = df[sub_col].map(lambda x: AUTO_MAP_DICT.get(x, (None, None, None))[0])
    df['记录类型'] = df['记录类型'].combine_first(mapped_types) # 如果原类型为空则填充
    # 对于债权类，强制覆盖 (防止被识别为普通支出)
    mask_force_type = mapped_types.isin(['应收款项', '应付款项', '收入'])
    if mask_force_type.any():
        df.loc[mask_force_type, '记录类型'] = mapped_types[mask_force_type]

    # 2. 映射 主类别 (只填充空值)
    mask_main = (df[main_col] == "")
    df.loc[mask_main, main_col] = df.loc[mask_main, sub_col].map(
        lambda x: AUTO_MAP_DICT.get(x, (None, None, None))[1]
    ).fillna("")

    # 3. 映射 项目 (只填充空值)
    mask_proj = (df['项目'] == "")
    df.loc[mask_proj, '项目'] = df.loc[mask_proj, sub_col].map(
        lambda x: AUTO_MAP_DICT.get(x, (None, None, None))[2]
    ).fillna("")
    ...
    if mask_found.any():
        ...
        # [核心优化点] 3. 同时清空项目和描述 (注意：此处【没有】清空'名称'，保留了字典匹配的成果)
...
def process_main(df, df_rules, main_col, sub_col):
    ...
    # ----------------------------------------------------
    # 注意：此处已删除原有的 trigger_words 清洗逻辑，统一移至函数末尾
    # ----------------------------------------------------
 ...
 # 6. 餐饮时段细化
    ...
    trigger_words = [
        "日用", "水果", "蔬菜", "饮料", "零食", "食材", "熟食", "正餐", "大件"
    ]
    
    # 1. 去前缀 (例如 "正餐. 北京烤鸭" -> "北京烤鸭")
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v11.15 (完整优化版) ==={bcolors.ENDC}")
    try:
