...
PATTERNS = {
    ...
    # 熟食
    'COOKED': r"(?:水煮花生|毛豆|藕夹|茄盒|锅包肉)",
    # 大米
    'RICE': r"(?:大米)",
    'MEAL': r"(?:三镇民生|兰州|丝路|沙县|永和四喜|水煎包|老乡鸡|混沌|馄饨|牛肉汤|热干面|黄蜀郎|鸡公煲|小吃|烧烤|食堂|路边摊|麦香园|长沙臭豆腐|东苑一层|东苑二层|西区食堂|包子|小笼包|外勤|板面|水饺|猪脚饭|肠粉|卤肉饭|油泼面|重庆小面|牛杂粉|麻辣香锅|麻辣烫|煎包|煎饼|烧饼|锅盔|馕)",
    # [关键] 垃圾字符库：只会删除这些特定的字符，而不会清空整句话
...
def process_heuristics(df, main_col, sub_col):
    ...
    # 6A. [新增] 大米 (高优先级)
    mask = check_keywords(PATTERNS['RICE'])
    ...
    # 6B. [新增] 熟食 (高优先级)
    mask = check_keywords(PATTERNS['COOKED'])
    mask = mask & (uncat | (df[main_col].isin(['购物', '居家'])))
    if mask.any():
        df.loc[mask, sub_col] = '食材'
        df.loc[mask, '名称'] = '熟食'
    ...
    # 6C. [新增] 豆制品 (高优先级)
    mask = check_keywords(PATTERNS['BEAN_PRODUCT'])
    mask = mask & (uncat | (df[main_col].isin(['购物', '居家'])))
    if mask.any():
        df.loc[mask, sub_col] = '食材'
        df.loc[mask, '名称'] = '豆制品'
    ...
    # 6D. [新增] 蔬菜 (高优先级)
    mask = check_keywords(PATTERNS['VEGETABLE'])
    mask = mask & (uncat | (df[main_col].isin(['购物', '居家'])))
    if mask.any():
        df.loc[mask, sub_col] = '食材'
        df.loc[mask, '名称'] = '蔬菜'
    ...
    # 6E. [升级] 食材与调料 (通用逻辑)
    # 只要商品名/备注里包含"酱料/榨菜/葱姜蒜"，就强制覆盖为"食材"，名称留空
    ...
    # 8. [重要修正] 垃圾信息清理 - 改为“替换删除”而非“整行清空”
    # 这样 "抖音电商-订单编号123.酱料" 变成 "抖音电商-.酱料"，保留了有用信息
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v10.18 (描述保留修正版) ==={bcolors.ENDC}")
    try:
