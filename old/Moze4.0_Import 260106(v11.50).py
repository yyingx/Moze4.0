"""
Moze 导入脚本 v11.50 (Clean & Stable)
Created on Sun Jan 05 2026
Optimized: Tue Jan 06 2026
Base: v39 (Logic) + v47 (Fixes)
@author: TZY_YX
"""
...
def process_main(df, df_rules, main_col, sub_col):
    ...
    df = df[(~df["当前状态"].isin(["已全额退款", "交易关闭"])) & (
        df["收/支"] == "支出") & (df["金额"] != 0)].copy()
    if df.empty:
        return pd.DataFrame()
...
def main():
    print(f"{BColors.BOLD}=== Moze 导入脚本 v11.50 (Clean & Stable) ==={BColors.ENDC}")
    try:
