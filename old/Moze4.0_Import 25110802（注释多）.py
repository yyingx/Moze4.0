...
class bcolors:
    """
    用于在终端输出中添加颜色的辅助类。
    (例如: print(f"{bcolors.OKGREEN}成功{bcolors.ENDC}"))
    """
...
def robust_date_converter(x):
    ...
    Args:
        x (any): 来自 Pandas 'apply' 的单个值。

    Returns:
        pd.Timestamp 或 pd.NaT: 转换后的日期时间对象。
    """
    ...
    # Excel 浮点数日期
        ...
        try:
            # 1899-12-30 是 Excel 日期系统的 'origin'
...
def get_user_inputs():
    ...
    Returns:
        tuple: (list[Path], datetime) 包含源文件列表和起始日期。
...
def load_rules(rule_path: Path):
    ...
    它会将精确规则和正则规则分开，并对精确规则去重。

    Args:
        rule_path (Path): Moze Dict.xlsx 文件的路径。

    Returns:
        pd.DataFrame: 处理和去重后的规则 DataFrame，如果失败则返回 None。
    """
...
def sniff_and_load_data(file_path: Path):
    ...
    此函数处理 微信(xlsx), 支付宝(csv), 以及合并后(xlsx) 三种格式。

    Args:
        file_path (Path): 单个源文件的路径。

    Returns:
        pd.DataFrame: 包含已加载和初步标准化数据 (含 '_source_tag') 的 DataFrame，
                      如果文件无法识别或加载失败，则返回 None。
    """
    ...
    if file_type is None:
        print(f"  错误：无法识别的文件类型。跳过此文件。", flush=True)
        if excel_workbook_object: excel_workbook_object.close()
        return None

    # --- 2. 加载 (Loading) 和规范化 (Normalization) 阶段 ---
    
    print(f"  正在加载数据 (跳过 {skip_rows} 行)...", flush=True)
    try:
        # 如果是 XLSX, io_source 是已打开的 workbook 对象; 否则是 file_path
        ...
        if file_type == "WeChat":
            ...
            df_source.rename(columns=ALIPAY_MAPPING, inplace=True) # 统一列名
...
def process_descriptions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df (pd.DataFrame): 包含 '描述', '商品', '备注' 等列的 DataFrame。

    Returns:
        pd.DataFrame: 带有已清理 '描述' 列的 DataFrame。
    """
    
    print("...[描述处理] 1/3: 设置默认值 (来自规则或商品)...", flush=True)
    # 如果'描述'为空，用'商品'列的内容填充
...
def _process_hardcoded_transfers(df_hardcoded_raw):
    """
    (辅助函数)
    专门处理所有硬编码的转账 (肖恩, 提现到工商银行等)。
    此函数封装了所有“1行变2行”的转账逻辑。

    Args:
        df_hardcoded_raw (pd.DataFrame): 筛选出的仅包含硬编码交易的 DataFrame。

    Returns:
        pd.DataFrame: 转换并格式化后的转账记录 (每笔原始交易变为2行)。
    """
    ...
    if not df_income_sean.empty:
        ...
        # A. 创建 "转出" (Transfer Out) 行 (零钱2 -> 零钱3)
        df_transfer_out = pd.DataFrame(index=df_income_sean.index)
        df_transfer_out['金额'] = pd.to_numeric(df_income_sean['金额(元)'], errors='coerce') * -1 # 负数
        df_transfer_out['记录类型'] = "转出"
        df_transfer_out['账户'] = "零钱2"
        # B. 创建 "转入" (Transfer In) 行 (零钱2 -> 零钱3)
        df_transfer_in = pd.DataFrame(index=df_income_sean.index)
        df_transfer_in['金额'] = pd.to_numeric(df_income_sean['金额(元)'], errors='coerce') # 正数
        df_transfer_in['记录类型'] = "转入"
        df_transfer_in['账户'] = "零钱3"
        
        df_transfers = pd.concat([df_transfer_out, df_transfer_in]) # 转出在前
    ...
    if not df_expense_sean.empty:
        ...
        # A. 创建 "转出" (Transfer Out) 行 (零钱3 -> 零钱2)
    ...
    if not df_icbc_raw.empty:
        ...
        # A. 创建 "转出" (Transfer Out) 行 (零钱3 -> 工行)
        df_transfer_out = pd.DataFrame(index=df_icbc_raw.index)
        df_transfer_out['金额'] = pd.to_numeric(df_icbc_raw['金额(元)'], errors='coerce') * -1 # 负数
        df_transfer_out['记录类型'] = "转出"
        df_transfer_out['账户'] = "零钱3"
        df_transfer_out['子类别'] = "提现" # (提现是一种特殊的转账)
        # B. 创建 "转入" (Transfer In) 行 (零钱3 -> 工行)
        df_transfer_in = pd.DataFrame(index=df_icbc_raw.index)
        df_transfer_in['金额'] = pd.to_numeric(df_icbc_raw['金额(元)'], errors='coerce') # 正数
        df_transfer_in['记录类型'] = "转入"
    ...
    if not list_of_transfer_dfs:
        return pd.DataFrame() # 没有处理任何记录

    # --- 格式化所有转账 ---
    df_all_transfers = pd.concat(list_of_transfer_dfs)
    
    df_all_transfers['主类别'] = "转账"
    df_all_transfers['子类别'] = df_all_transfers['子类别'].fillna("转账") # '提现' 会被保留，其他NaN变为'转账'
    ...
    df_all_transfers['_Sort_Date'] = df_all_transfers['交易时间'] # 排序用的原始时间戳
...
def _apply_meal_logic(df: pd.DataFrame) -> pd.DataFrame:
    """
    (辅助函数)
    应用餐饮时间特殊逻辑：根据特定商家和交易时间自动分类为早/中/晚餐。

    Args:
        df (pd.DataFrame): 待处理的 DataFrame。

    Returns:
        pd.DataFrame: 更新了 '主类别', '子类别', '项目' 的 DataFrame。
    """
    print("  ...正在应用餐饮时间特殊逻辑...", flush=True)
    # (优化) 使用非捕获组 (?:...)
    merchant_pattern = r"(?:三镇民生|兰州|丝路|沙县|永和四喜|水煎包|老乡鸡|牛肉汤|热干面馆|黄蜀郎|鸡公煲|彩凤小吃店|烧烤|食堂|路边摊|麦香园|长沙臭豆腐)"
    merchant_mask_old = df['商家(old)'].str.contains(merchant_pattern, na=False, regex=True)
    merchant_mask_new = df['商家'].str.contains(merchant_pattern, na=False, regex=True)
    merchant_mask = merchant_mask_old | merchant_mask_new
    
    item_pattern = r"(?:东苑一层|西区食堂)"
    item_mask = df['商品'].str.contains(item_pattern, na=False, regex=True)
    
    # 必须是特定商家或商品，且记录类型为 '支出'
    condition_mask = (merchant_mask | item_mask) & (df['记录类型'] == '支出')
    
    if not condition_mask.any():
        return df # 没有任何餐饮记录，跳过

    hour = df.loc[condition_mask, '交易时间'].dt.hour
    ...
    # 使用 numpy.select 高效分类
    df.loc[condition_mask, 'temp_meal_subcat'] = np.select(condlist, choicelist_subcat, default=pd.NA)
    
    # 仅更新那些成功匹配到餐点时间 (非 NA) 的行
    final_meal_mask = condition_mask & df['temp_meal_subcat'].notna()
    
    df.loc[final_meal_mask, '主类别'] = '饮食'
    df.loc[final_meal_mask, '项目'] = '食'
    df.loc[final_meal_mask, '子类别'] = df['temp_meal_subcat']
    
    df.drop(columns=['temp_meal_subcat'], inplace=True, errors='ignore')
    return df

def _apply_charging_logic(df: pd.DataFrame) -> pd.DataFrame:
    """
    (辅助函数)
    应用充电桩特殊逻辑：自动分类。
    ...
    Returns:
        pd.DataFrame: 更新了 '名称', '主类别', '子类别' 的 DataFrame。
    """
    print("  ...正在应用充电桩特殊逻辑 (分类)...", flush=True)
    is_charging_flag = df['商品'].str.contains("自助服务-充电桩", na=False)
    
    if is_charging_flag.any():
        df.loc[is_charging_flag, ['名称', '主类别', '子类别']] = ['充电', '交通', '加油充电']
...
def _process_main_transactions(df_main_raw, df_rules):
    """
    (辅助函数)
    专门处理所有 "非硬编码" 的记录 (收入, 支出, /)。
    此函数包含所有规则匹配和分类逻辑 (查字典)。

    Args:
        df_main_raw (pd.DataFrame): 筛选出的所有常规交易。
        df_rules (pd.DataFrame): 从 Moze Dict 加载的规则。

    Returns:
        pd.DataFrame: 经过规则匹配、分类和格式化后的 DataFrame。
    """
    ...
    df_expenses = df_main_raw[status_condition].copy()
    ...
    df_target = pd.merge(
        ...
        on='商家(old)', 
        how='left' # 'left' 保证所有交易行被保留
    )
    # 确保所有 '_rule' 列都存在，即使 merge 没有匹配到
    ...
    if unmatched_mask.any() and not df_rules_regex.empty:
        ...
        for _, rule in df_rules_regex.iterrows():
            pattern = rule['商家(old)']
            try:
                # (关键) 这里的 str.contains() 就是之前触发警告的地方。
                # 确保 '商家(old)' 列中 (is_regex=1) 的规则
                # 使用非捕获组 (?:...) 而不是 (...)
    ...
    # 2D. 设置默认值
    # 如果规则中没有指定'记录类型'，则使用原始的 '收/支' (即 '收入', '支出', '/')
    df_target['记录类型'] = df_target['记录类型'].fillna(df_target['收/支'])
    # 如果规则中没有指定'商家'，则使用原始的 '交易对方'
    df_target['商家'] = df_target['商家'].fillna(df_target['交易对方'])

    # 2E. 应用特殊逻辑 (已封装)
    df_target = _apply_meal_logic(df_target)
    df_target = _apply_charging_logic(df_target)
    ...
    df_target['_Sort_Date'] = df_target['交易时间'] # 排序用的原始时间戳
...
def process_transactions(df_source, df_rules, start_datetime_obj):
    ...
    它负责：
    1. 预处理和时间筛选。
    2. "分诊"(Triage)：将交易分为“硬编码转账”和“常规交易”。
    3. 调用辅助函数分别处理。
    4. 合并并应用最终清理 (如标签)。

    Args:
        df_source (pd.DataFrame): 从所有文件合并的原始 DataFrame。
        df_rules (pd.DataFrame): 已加载的规则字典。
        start_datetime_obj (datetime): 筛选用的起始日期 (或 None)。

    Returns:
        pd.DataFrame: 准备好导出为 CSV 的最终 DataFrame。
    """
    ...
    # (定义) 哪些交易需要被硬编码处理 (1行变2行)
    sean_mask = (df_source["交易对方"] == "肖恩")
    icbc_mask = (df_source["交易对方"] == "工商银行(9579)")
    
    hardcoded_mask = sean_mask | icbc_mask 
    
    df_transfers_raw = df_source[hardcoded_mask].copy() # 1. 需要硬编码的
    df_main_raw = df_source[~hardcoded_mask].copy()      # 2. 其他所有
    ...
    df_final['标签'] = df_final['标签'].str.replace(r'\s+', ' ', regex=True) # 压缩多余空格
    
    print("...向量化处理完成。", flush=True)
    
    # 确保列是完整的，并且丢弃临时列 (如 _Sort_Date)
...
def main():
    ...
    try:
        ...
        # 创建一个临时的排序键: '转出' = 0, '转入' = 1, 其他 = 9
        # 这确保了在时间戳相同时，'转出' 始终在 '转入' 之前
        sort_key_map = {
            '转出': 0,
            '转入': 1
        }
        ...
        df_final.sort_values(
            by=["_Sort_Date", "_Transfer_Sort_Key"],  # 1. 按日期排 2. 按转账键排
            ascending=[False, True], # 日期 降序 (新->旧), 转账键 升序 (0->1)
            inplace=True,
            kind='stable' # 保证排序的稳定性
        )
        ...
        df_final.reset_index(drop=True, inplace=True) # 重置索引，以便进行行号检查
        ...
        if uncategorized_count > 0:
            ...
            for row in problem_rows_uncat.head(10).itertuples():
                csv_row_num = row.Index + 2 # CSV行号 (包含抬头)
        ...
        df_final.to_csv(
            target_path, 
            index=False, 
            columns=TARGET_HEADERS, # 仅保留目标列
            encoding='utf-8-sig' # 确保 Excel 能正确打开中文
        )
