...
def process_descriptions(df: pd.DataFrame, main_cat_col: str, sub_cat_col: str) -> pd.DataFrame:
    
    print("...[描述处理] 1/3: 应用充电桩描述逻辑...", flush=True)
    ...
    # *** (!!!) 新增：软件/水果/饮料 描述分类逻辑 (!!!) ***
    # ******************************************************
    print("...[描述处理] 2/3: 应用软件/水果/饮料特殊分类...", flush=True)
    
    # 确保列存在，以防万一
    if '名称' not in df.columns: df['名称'] = pd.NA
    if '项目' not in df.columns: df['项目'] = pd.NA
    
    # --- 规则 1：软件 (NEW) ---
    software_pattern = r"软件"
    software_mask = df['描述'].astype(str).str.contains(software_pattern, na=False, regex=True)
    
    if software_mask.any():
        df.loc[software_mask, sub_cat_col] = 'Software'
        df.loc[software_mask, main_cat_col] = '虚拟'
        df.loc[software_mask, '项目'] = '学习'
        # 注意：这里不设置'名称'，按您的要求

    # --- 规则 2：水果 ---
    # (!!!) 修正：确保不覆盖软件规则
    fruit_pattern = r"(?:水果|果|苹果|香蕉|枣|桃|梨|柑|草莓|甘蔗|橘|桔)"
    fruit_mask = df['描述'].astype(str).str.contains(fruit_pattern, na=False, regex=True) & (~software_mask)
    
    if fruit_mask.any():
        df.loc[fruit_mask, '名称'] = '水果'
        df.loc[fruit_mask, sub_cat_col] = '饮料水果'
        df.loc[fruit_mask, main_cat_col] = '饮食'
        df.loc[fruit_mask, '项目'] = '食'

    # --- 规则 3：饮料 ---
    # (!!!) 修正：确保不覆盖软件和水果规则
    drink_pattern = r"(?:可乐|红牛|奶茶|东鹏|饮料)"
    drink_mask = df['描述'].astype(str).str.contains(drink_pattern, na=False, regex=True) & (~fruit_mask) & (~software_mask)
    
    if drink_mask.any():
        df.loc[drink_mask, '名称'] = '饮料'
        df.loc[drink_mask, sub_cat_col] = '饮料水果'
        df.loc[drink_mask, main_cat_col] = '饮食'
        df.loc[drink_mask, '项目'] = '食'
...
def _process_main_transactions(df_main_raw, df_rules, main_cat_col, sub_cat_col):
    ...
    if "收/支" in df_main_raw.columns:
        # 只保留“支出”的行，过滤掉“收入”和“不计收支”
        income_expense_condition = (df_main_raw["收/支"] == "支出")
    ...
    if count_filtered > 0:
         print(f"  ...已通过筛选 (状态/收支类型) 过滤掉 {count_filtered} 条记录。", flush=True)
    ...
    # (!!!) 修正：默认描述逻辑已移到此处
    print("  ...正在设置默认描述 (来自'商品'列)...", flush=True)
    if '描述' not in df_target.columns:
        df_target['描述'] = pd.NA 
    if '商品' not in df_target.columns:
        df_target['商品'] = pd.NA
    df_target['描述'] = df_target['描述'].fillna(df_target['商品'])
    ...
    # 支付宝的特殊逻辑 (T200P4 / 描述合并)
    print("  ...正在应用支付宝特殊逻辑 (淘宝订单 / 描述合并)...", flush=True)
    
    # 1. 如果“商家订单号”前面五位=T200P4，目标文件“商家”=淘宝
    if '商户单号' in df_target.columns:
        is_alipay_mask = (df_target['_source_tag'] == '#AliPay')
        # 确保 '商户单号' 是字符串
        is_taobao_order_mask = df_target['商户单号'].astype(str).str.startswith('T200P4', na=False)
        final_taobao_mask = is_alipay_mask & is_taobao_order_mask
        
        if final_taobao_mask.any():
            if '商家' not in df_target.columns: df_target['商家'] = pd.NA
            df_target.loc[final_taobao_mask, '商家'] = '淘宝'

    # 2. 如果描述<>空值 目标文件“描述”= 字典的描述&&“.”&&支付宝账单中的描述(备注)
    # (此时 '描述' 列 = 字典的描述 OR 商品, '备注' 列 = 支付宝账单中的描述)
    if '描述' in df_target.columns and '备注' in df_target.columns:
        is_alipay_mask = (df_target['_source_tag'] == '#AliPay')
        # 字典描述不为空
        dict_desc_exists = df_target['描述'].notna() & (df_target['描述'] != "")
        # 支付宝账单备注不为空
        alipay_memo_exists = df_target['备注'].notna() & (df_target['备注'] != "")

        final_concat_mask = is_alipay_mask & dict_desc_exists & alipay_memo_exists
        
        if final_concat_mask.any():
            # (!!!) 修正：合并 字典描述 和 支付宝备注
            df_target.loc[final_concat_mask, '描述'] = df_target.loc[final_concat_mask, '描述'] + '.' + df_target.loc[final_concat_mask, '备注']
            
    # 2F. 描述处理
    df_target = process_descriptions(df_target, main_cat_col, sub_cat_col)
...
def process_transactions(df_source, df_rules, start_datetime_obj, target_headers_list, main_cat_col, sub_cat_col):
    ...
    # (核心修正) 在筛选前，清理“交易对方”列中的空白
    df_source['交易对方'] = df_source['交易对方'].astype(str).str.strip()
