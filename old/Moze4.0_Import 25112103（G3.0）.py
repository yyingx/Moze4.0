...
def process_heuristics(df, main_col, sub_col):
    ...
    # --- [修复] 3. 充电桩 (独立逻辑) ---
    # 逻辑A：提取描述 (所有充电桩都执行，不管是否已分类)
    is_charging = df['商品'].str.contains(CONFIG['KEYWORD_CHARGING'], na=False)
    # 确保有 '/' 才分割，防止报错
    has_slash = is_charging & df['商品'].str.contains('/', na=False)
    if has_slash.any():
        split = df.loc[has_slash, '商品'].str.split('/', n=1, expand=True)
        if split.shape[1] > 1:
            df.loc[has_slash, '描述'] = split[1].str.strip()

    # 逻辑B：自动分类 (仅针对未分类)
    mask_cat = is_charging & uncat
    if mask_cat.any(): 
    ...
    # 5. 水果
    pat = r"(?:水果|果园|果蔬|百果园|鲜果|果业|苹果|香蕉|红枣|大枣|桃子|水蜜桃|樱桃|黄桃|香梨|雪梨|柑橘|沃柑|草莓|甘蔗|桔子|沙糖桔|葡萄|哈密瓜|西瓜|甜瓜|柚子|柠檬|橙子|菠萝|凤梨|榴莲|山竹|蓝莓|荔枝|龙眼|椰子|柿子|杨梅|李子|芒果|猕猴桃|火龙果|百香果|莲雾|车厘子)"
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v7.3 (充电桩修复版) ==={bcolors.ENDC}")
    try:
