...
PATTERNS = {
    ...
    # [核心升级] 垃圾字符库：增加了 单号、备注、方备注
    'CLEAN': r"(?:2\.00E\+15|经营码交易|余额充值|.*?后付|消费|费用|支付|商户|二维码收款|收款|美团|编号|转账|错误|订单编号|单号|备注|方备注|\d{10,}|/|[A-Za-z]\d+-\d+)"
}
...
def process_heuristics(df, main_col, sub_col):
    ...
    # 6. 大类识别
    ordered_checks = [('VEGETABLE', '蔬菜'), ('RICE', '大米'), ('BEAN_PRODUCT', '豆制品'), ('COOKED', '熟食')]
    for key, name in ordered_checks:
    ...
    mask = mask & (~mask_already_set) & (uncat | (df[main_col].isin(['购物', '居家', '饮食'])))
    if mask.any():
        df.loc[mask, sub_col] = '食材'
        df.loc[mask, '名称'] = "" 
        df[sub_col] = df[sub_col].replace("nan", "")
    ...
    # 9. [核心升级] 强力垃圾清洗
    # 步骤A：使用正则替换掉大部分垃圾
    # 步骤B：[新增] 使用 strip 去除首尾残留的符号 (如 "=-康福路店" -> "康福路店")
    
    # 跳过充电桩保护 (如果名称是充电，就不洗，防止洗掉编号)
    safe_to_clean = df['名称'] != '充电'
    
    if safe_to_clean.any():
        # A. 正则替换
        df.loc[safe_to_clean, '描述'] = df.loc[safe_to_clean, '描述'].astype(str).str.replace(PATTERNS['CLEAN'], ' ', regex=True)
        # B. 首尾符号清理 (去除 . - _ = + : / 以及中文冒号)
        df.loc[safe_to_clean, '描述'] = df.loc[safe_to_clean, '描述'].str.strip(" .-_=+:：/")
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v10.31 (极致去噪美化版) ==={bcolors.ENDC}")
    try:
