...
# 这里的 CLEAN 正则不再用于最终的描述清洗，仅用于保护性的 combo_punch 函数(如果有其他地方用到)
...
def process_main(df, df_rules, main_col, sub_col):
    ...
    # [关键修正] 移除正则清洗 (apply_combo_punch)，仅保留去除多余空格
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v11.03 (备注绝对原样版) ==={bcolors.ENDC}")
    try:
