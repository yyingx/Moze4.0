...
if (CURRENT_DIR / "Moze Dict.xlsx").exists():
...
CONFIG = {
    'TRANSFER_TARGET_1': '肖恩', 'TRANSFER_TARGET_2': '工商银行(9579)',
    'CARD_PINGAN': '平安银行信用卡(4946)', 'CARD_ICBC': '工商银行储蓄卡(9579)',
    'CARD_CMB': '招商银行储蓄卡(3379)', 'CARD_CGB': '广发银行信用卡(4826)',
    'KEYWORD_CHARGING': '自助服务-充电桩', 'KEYWORD_ALIPAY_CSV': '支付宝支付科技有限公司',
...
DATA_SOURCE = {
    ...
    'INGREDIENTS': [
        "食材",
        "馒头", "火腿","腊肠", "榨菜",  "老干妈", "杂酱", "生水饺", "酱料", "炸酱",  "白砂糖", "蒸肉粉", "火锅底料",
    ...
    'MEAL': [
        ...
        "热干面", "太和板面", "板面", "油泼面", "牛肉面", "刀削面", "牛杂粉", "肠粉", "牛肉汤",
        ...
        # 6. 通用场景/店名 (特征较弱，放在最后兜底)
        "烧烤", "路边摊", "食堂", "餐厅", "早点", "小吃", "餐饮"
    ],
    'Parking_fee': ["WF7023"],
    'DAILY_NECESSITIES': [
        "日用", 
        "抽纸", "卷纸", "厨房纸","垃圾袋", "保鲜袋", "保鲜膜","洗衣液", "洗洁精","牙膏", "洗发水""一次性手套", "一次性杯", "棉签"
    ],
...
INGREDIENT_PRIORITY = [
    ...
    ('COOKED', '熟食', '食材'), ('Snack', '', '零食'), ('Clothing_Shoes_Bags', '', '服饰鞋包'),
    ('Furniture_HomeTextiles', '', '家具家纺'), ('SERVER', '节点', '虚拟其他'), ('Adult_Products', 'Condoms', '保健用品')
]

# (记录类型, 主类, 项目)
RAW_MAPPING_CONFIG = {
    ('支出', '饮食', '食'): ['午餐', '夜宵', '早餐', '晚餐', '纯净水', '零食', '食材', '饮料水果', '熟食'], 
    ('支出', '购物', '日用&家用'): ['家具家纺', '数码电器', '日常用品', '服饰鞋包'],
...
PATTERNS = {k: r"(?:" + "|".join(map(re.escape, v)) + ")" for k, v in DATA_SOURCE.items()}

AUTO_MAP_DICT = {}
for (r_type, main, proj), sub_list in RAW_MAPPING_CONFIG.items():
    for sub in sub_list:
        AUTO_MAP_DICT[sub] = (r_type, main, proj)
...
def clean_final_description(df):
    """最终清洗：去前缀 & 去冗余词 (放在分类后执行)"""
    trigger_words = ["日用", "水果", "蔬菜", "饮料", "零食", "食材", "熟食", "正餐"]
    
    # 1. 去前缀
    keywords_regex = "|".join(trigger_words)
    prefix_pat = rf"^(?:{keywords_regex})[. 。\s-]+(.*)"
    df['描述'] = df['描述'].astype(str).str.replace(prefix_pat, r'\1', regex=True).str.strip()

    # 2. 去冗余
    mask_redundant = df['描述'].isin(trigger_words) | (df['描述'] == "")
...
def robust_date_converter(x):
    ...
    try: return pd.to_datetime(x, unit='D', origin='1899-12-30') if isinstance(x, (int, float)) else pd.to_datetime(x, errors='coerce')
...
def load_rules(rule_path: Path):
    ...
    try:
        ...
        s_name = next((s for s in xl.sheet_names if s.lower()!='settings' and '商家(old)' in pd.read_excel(rule_path, sheet_name=s, nrows=1).columns), None)
    ...
    try:
        ...
        if file_path.suffix.lower() == '.csv':
            for enc in ['gb18030', 'utf-8']:
                try:
                    preview = pd.read_csv(file_path, header=None, nrows=50, encoding=enc, names=list(range(30)))
                    mask = preview.apply(lambda x: x.astype(str).str.contains('交易创建时间|交易时间|付款时间', na=False)).any(axis=1)
        ...
        df.columns = df.columns.astype(str).str.strip().str.replace('（', '(', regex=False).str.replace('）', ')', regex=False)
        ...
        df['金额(元)'] = pd.to_numeric(
            df['金额(元)'].astype(str).str.replace('¥', '', regex=False).str.replace(',', '', regex=False).str.strip(), 
            errors='coerce'
        ).fillna(0)
...
def ensure_columns(df, main_col, sub_col):
    for c in [main_col, sub_col, '金额', '记录类型', '账户', '商家', '描述', '项目', '名称', '对象', '标签', '日期', '时间', '币种', '手续费', '折扣']:
        if c not in df.columns: df[c] = pd.NA
    for c in ['记录类型', '项目', '对象', main_col, sub_col, '商家', '名称']: df[c] = df[c].fillna("")
    return df
...
def process_transfers(df, main_col, sub_col):
    ...
    def add_pair(sub_df, out_a, in_a, sub_val='转账'):
        ...
        o, i = sub_df.copy(), sub_df.copy()
        o['金额'] = pd.to_numeric(o['金额(元)']) * -1; o['记录类型']='转出'; o['账户']=out_a; o[sub_col]=sub_val
        i['金额'] = pd.to_numeric(i['金额(元)']); i['记录类型']='转入'; i['账户']=in_a; i[sub_col]=sub_val
        res.extend([o, i])
...
def process_heuristics(df, main_col, sub_col):
    ...
    for col in [sub_col, '项目', '描述', '商家', '商品']: df[col] = df[col].astype(str).str.strip().replace("nan", "")
    
    def check(k): return df['商家'].str.contains(PATTERNS.get(k,"^$"), regex=True) | df['商品'].str.contains(PATTERNS.get(k,"^$"), regex=True) | df['描述'].str.contains(PATTERNS.get(k,"^$"), regex=True)

    uncat = (df[sub_col] == "")
    
    # 1. 关键词分类
    mask = df['商品'].str.contains(CONFIG['KEYWORD_CHARGING'], na=False) & uncat
    if mask.any(): df.loc[mask, ['名称', sub_col]] = ['充电', '加油充电']
    ...
    for k, n, s in [('FRUIT','水果','饮料水果'), ('DRINK','饮料','饮料水果'), ('WATER','','纯净水')]:
        ...
        if mask.any(): df.loc[mask, ['名称', sub_col]] = [n, s]

    mask_meal = check('MEAL')
    for key, name, sub_c in INGREDIENT_PRIORITY:
        mask = check(key) & (~mask_meal) & (uncat | (df[main_col].isin(['购物', '居家', '饮食'])))
        if key in ['SEAFOOD', 'PORK', 'POULTRY', 'BEEF_MUTTON', 'VEGETABLE']: mask &= (~check('COOKED'))
        if mask.any(): df.loc[mask, [sub_col, '名称']] = [sub_c, name]
# [修改点] 移除原来的 daily_s / daily_w 分别处理，改为统一处理
    daily_keywords = DATA_SOURCE['DAILY_NECESSITIES']
    pat_daily = r"(?:" + "|".join(map(re.escape, daily_keywords)) + ")"
    
    # 获取掩码：包含日用词 且 不是餐饮
    def get_mask(p): return (df['商家'].str.contains(p, regex=True)|df['商品'].str.contains(p, regex=True)|df['描述'].str.contains(p, regex=True))
    mask_daily = get_mask(pat_daily) & (~mask_meal)

#     daily_s = [w for w in DATA_SOURCE['DAILY_NECESSITIES'] if len(w)>1]
#     daily_w = [w for w in DATA_SOURCE['DAILY_NECESSITIES'] if len(w)==1]
#     pat_s = r"(?:" + "|".join(map(re.escape, daily_s)) + ")"
#     pat_w = r"(?:" + "|".join(map(re.escape, daily_w)) + ")"
#     
#     def get_mask(p): return (df['商家'].str.contains(p, regex=True)|df['商品'].str.contains(p, regex=True)|df['描述'].str.contains(p, regex=True))
#     
#     mask_s = get_mask(pat_s) & (~mask_meal)
#     if mask_s.any(): df.loc[mask_s, [sub_col, '名称']] = ['日常用品', '']
# 
#     mask_w = get_mask(pat_w) & (df[sub_col]!='日常用品') & (uncat|df[main_col].isin(['购物','居家','饮食'])) & (~df[sub_col].isin(['食材','饮料水果'])) & (~mask_meal)
#     if mask_w.any(): df.loc[mask_w, [sub_col, '名称']] = ['日常用品', '']
# 
    # 停车费
    mask = check('Parking_fee')
    if mask.any(): df.loc[mask, [sub_col, '名称', '对象']] = ['报账', '停车费', '天之逸']
    ...
    if mask_found.any():
        ...
        if mask_name.any(): df.loc[mask_name, '对象'] = extracted[1][mask_name].str.strip()
...
def process_main(df, df_rules, main_col, sub_col):
    ...
    # 1. 支付方式标准化
    pay_map = {re.escape(CONFIG[k]): v for k, v in [('CARD_PINGAN','平安银行4946'),('CARD_ICBC','工商银行'),('CARD_CMB','招商银行Ⅱ'),('CARD_CGB','广发银行4826')]}
    pay_map[r'^零钱.*'] = '零钱3'
    df['支付方式'] = df['支付方式'].astype(str).replace(pay_map, regex=True)
    ...
    df = pd.merge(df, df_rules[df_rules['is_regex']==0].rename(columns=rename_map).drop(columns='is_regex', errors='ignore'), on='商家(old)', how='left')
    
    uncat = df[df[f"{main_col}_rule"].isna()].index if f"{main_col}_rule" in df.columns else df.index
    if not uncat.empty:
        to_match = df.loc[uncat, '交易对方'].astype(str)
        for _, r in df_rules[df_rules['is_regex']==1].iterrows():
            idx = to_match[to_match.str.contains(r['商家(old)'], regex=True)].index
            if not idx.empty:
                for c, rc in rename_map.items(): df.loc[idx, rc] = r[c]
                to_match = to_match.drop(idx)

    for orig, rule in rename_map.items():
        if rule in df.columns:
            if orig != '描述': df[orig] = df[rule].combine_first(df[orig])
            if orig != '描述': df.drop(columns=[rule], inplace=True) 

    # 3. 淘宝识别
    if '商户单号' in df.columns:
        mask_tb = (df.get('_source_tag', '') == '#AliPay') & \
                  df['商户单号'].astype(str).str.strip().str.startswith('T200P4', na=False)
        if mask_tb.any():
            df.loc[mask_tb, '商家'] = '淘宝'

    # 4. 构建描述
    df = construct_description(df)
    if '描述_rule' in df.columns: df.drop(columns=['描述_rule'], inplace=True)
    ...
    if mask_time.any():
        ...
        df.loc[mask_time, sub_col] = np.select([(h>=6)&(h<11), (h>=11)&(h<16), (h>=16)&(h<21)], ["早餐", "午餐", "晚餐"], default="夜宵")
    ...
    df.loc[(df['记录类型'].isin(['支出','应付款项','应收款项'])) & (df.get('收/支')=='支出'), '金额'] *= -1
    ...
    df['账户'] = df.get('支付方式', ""); df.loc[:, ['币种','手续费','折扣']] = ["CNY", 0, 0]
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v11.33 (纯净排序版) ==={bcolors.ENDC}")
    try:
        ...
        mask_trans = df_raw['交易对方'].isin([CONFIG['TRANSFER_TARGET_1'], CONFIG['TRANSFER_TARGET_2']])
        res_dfs = []
        ...
        if '标签' in df_final.columns:
            ...
            df_final.loc[m, '标签'] = (df_final.loc[m, '标签'].fillna("") + " " + df_final.loc[m, '_source_tag']).str.strip()

        # [修正] 只针对转账逻辑进行微调：转出(0) -> 转入(1)
        # 其他类型(支出/收入等)不参与特殊排序，默认为 0 (fillna(0))
        # 这样 转出 和 支出 会混在一起(按时间)，而 转入 会排在它们后面
        type_priority = {
            '转出': 0,
            '转入': 1
        }
        df_final['_Type_Rank'] = df_final['记录类型'].map(type_priority).fillna(0)

        # 排序：时间(新->旧), 优先级(0->1)
        df_final.sort_values(['_Sort_Date', '_Type_Rank'], ascending=[False, True], inplace=True)
        
        if not TARGET_DIR.exists(): TARGET_DIR.mkdir(parents=True, exist_ok=True)
        path = TARGET_DIR / f"MOZE导入_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        df_final.to_csv(path, index=False, columns=cols, encoding='utf-8-sig')
        
        print(f"\n{bcolors.OKGREEN}成功! 文件: {path}{bcolors.ENDC}")
        
        for name, mask, check_cols in [('支出', df_final['记录类型']=='支出', [main_col, sub_col, '项目']), ('债权', df_final['记录类型'].isin(['应收款项','应付款项']), [main_col, sub_col, '对象'])]:
            bad = mask & df_final[check_cols].isin(["", pd.NA]).any(axis=1)
            if bad.any():
                print(f"{bcolors.FAIL}[严重] {bad.sum()} 条【{name}】记录缺失关键信息:{bcolors.ENDC}")
                print(df_final.loc[bad, ['日期', '商家', '描述', '金额'] + check_cols].head().to_string())
            else: print(f"{bcolors.OKGREEN}√ {name}记录完美{bcolors.ENDC}")
