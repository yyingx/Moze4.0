...
# --- [配置区 3] 正则表达式库 (已扩充) ---
PATTERNS = {
    # [增强] 包含更多水果种类、热带水果、浆果及常见店名
    'FRUIT': r"(?:水果|果园|果蔬|果品|百果园|鲜丰|鲜果|果业|苹果|香蕉|红枣|大枣|桃子|水蜜桃|樱桃|黄桃|香梨|雪梨|柑橘|沃柑|草莓|甘蔗|桔子|沙糖桔|葡萄|提子|哈密瓜|西瓜|甜瓜|羊角蜜|柚子|柠檬|橙子|菠萝|凤梨|榴莲|山竹|蓝莓|荔枝|龙眼|椰子|柿子|杨梅|李子|芒果|猕猴桃|火龙果|百香果|莲雾|车厘子|牛油果|鳄梨|芭乐|番石榴|菠萝蜜|桑葚|枇杷|杨桃|无花果|圣女果|小番茄|姑娘果|西梅|青枣|冬枣|黑布林|人参果|释迦)",
    'DRINK': r"(?:可乐|红牛|奶茶|东鹏|饮料|汽水|果汁|椰汁|酸奶)",
    'WATER': r"(?:纯净水|矿泉水|农夫山泉|怡宝|百岁山)",
    ...
    'CLEAN': r"(?:2\.00E\+15|/|经营码交易|余额充值|.*?后付|消费|费用|支付|商户|二维码收款|收款|美团|编号|转账|错误|订单编号|\d{10,})"
}
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v10.7 (细节增强版) ==={bcolors.ENDC}")
    try:
        ...
        else: print(f"{bcolors.OKGREEN}√ 支出记录完美{bcolors.ENDC}")
        ...
        else: print(f"{bcolors.OKGREEN}√ 应收款项完美{bcolors.ENDC}")
