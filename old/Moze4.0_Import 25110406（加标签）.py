...
# ******************************************************
# *** (已更新) 'sniff_and_load_data' 函数 ***
# *** (此版本增加了 _source_tag 内部标签) ***
# ******************************************************
def sniff_and_load_data(file_path: Path):
    ...
    try:
        ...
        if file_type == "WeChat":
            ...
            df_source['_source_tag'] = '#WechatPay' # (新增) 添加来源标签
            ...
            df_source['_source_tag'] = '#Alipay' # (新增) 添加来源标签
            ...
            df_source['_source_tag'] = '' # (新增) 合并文件没有特定来源
...
def process_transactions(df_source, df_rules, start_datetime_obj):
    ...
    # (重要) df_target 现在包含了 `_source_tag` 列
    ...
    condlist = [
        (hour >= 6) & (hour < 10),   # breakfast_mask
        (hour >= 11) & (hour < 16),  # lunch_mask
        (hour >= 16) & (hour < 21),  # dinner_mask
        (hour >= 21) | (hour < 2)    # MidnightSnack_mask
    ]
    ...
    final_meal_mask = (
        condition_mask & 
        is_expense_mask & 
        df_target['temp_meal_subcat'].notna()
    )
    ...
    if '_source_tag' not in df_target.columns:
        df_target['_source_tag'] = ""
        
    # 1. 填充空值
    df_target['_source_tag'] = df_target['_source_tag'].fillna("")
    df_target['标签'] = df_target['标签'].fillna("") # fill_na_cols 已经做过

    # 2. 仅为 "支出" 记录准备来源标签
    is_expense_mask = (df_target['记录类型'] == '支出')
    source_tags_to_apply = df_target['_source_tag'].where(is_expense_mask, "")

    # 3. 合并 (使用 .str.cat 确保空格正确)
    # (这会处理 "有规则标签" + "有来源标签" 和 "无规则标签" + "有来源标签")
    df_target['标签'] = df_target['标签'].str.cat(source_tags_to_apply, sep=' ').str.strip()
    # 用 ' ' 替换掉多余的空格 (如果规则标签和来源标签之间有多个空格)
    df_target['标签'] = df_target['标签'].str.replace(r'\s+', ' ', regex=True)
