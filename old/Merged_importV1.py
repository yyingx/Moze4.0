...
try:
    ...
    for index, row in df_expenses.iterrows():
            ...
            new_row["子类别"] = ""
            new_row["名称"] = ""
            new_row["商家"] = merchant_name        # 默认 = 交易对方
            ...
            # 规则 1 (新): 检查 "商品" 列
            if "自助服务-充电桩" in item_description:
                # 1.1: 它是充电, 先设置分类
                new_row["名称"] = "充电"
                new_row["主类别"] = "交通"
                new_row["子类别"] = "加油充电"
                
                # 1.2: 检查 "商品" 列中是否有斜杠, 以便拆分
                if "/" in item_description:
                    parts = item_description.split('/', 1)
                    
                    # 检查分割是否成功
                    if len(parts) == 2 and parts[1].strip() != "":
                        description_clean = parts[1].strip() # 斜杠后
                        
                        # 覆盖 描述
                        new_row["描述"] = description_clean # 描述 = G371431-2
                    # (如果分割失败, 描述保持默认的 item_description)
                # (如果 is_charging=True 但没有 '/', 描述保持默认的 item_description)

            # 规则 2 (旧): "自助服务平台" (检查 商家 列)
            elif "自助服务平台" in merchant_name:
                new_row["名称"] = "充电"
                new_row["主类别"] = "交通"
                new_row["子类别"] = "加油充电"
                # (描述保持默认值)
