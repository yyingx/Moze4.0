...
DATA_SOURCE = {
    ...
    'DRINK': [
        "可乐", "红牛", "奶茶", "东鹏", "饮料", "汽水", "果汁", "椰汁", "酸奶", "咖啡", "拿铁", "美式"
    ],
    'WATER': [
...
# [修复] 去掉 SOFTWARE_EXTRACT 外层括号，消除 UserWarning
...
def process_heuristics(df, main_col, sub_col):
    ...
    # [修正] 确保 COOKED 熟食在最后，覆盖 SEAFOOD 等
    mask_is_cooked = check_keywords('COOKED') # 额外加一个熟食判断，辅助生鲜判断
    ...
    for key, name, sub_c in ordered_checks:
        ...
        # [关键] 如果是生鲜(海鲜/肉)，必须排除掉熟食
        # 这样 "炸带鱼" 不会被识别为 "海鲜"
        if key in ['SEAFOOD', 'PORK', 'Raw chicken', 'BEEF_MUTTON']:
    ...
    # 10. 软件名称净化 (使用 str.extract 配合 括号捕获)
    mask_sw = df['描述'].str.contains(PATTERNS['SOFTWARE_EXTRACT'], regex=True, na=False)
    if mask_sw.any():
        # 这里需要临时加上括号以支持 extract
        extract_pattern = f"({PATTERNS['SOFTWARE_EXTRACT']})"
        extracted = df.loc[mask_sw, '描述'].str.extract(extract_pattern, expand=False)
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v10.74 (警告修复 + 熟食优先修正版) ==={bcolors.ENDC}")
    try:
