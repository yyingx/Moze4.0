...
def process_transactions(df_source, df_rules, start_datetime_obj):
    ...
    if not df_income_raw.empty:
        ...
        if not df_income_sean.empty:
            print(f"  检测到 {len(df_income_sean)} 条来自 '肖恩' 的收入，将处理为转账...", flush=True)
            
            # (已修复) 转账逻辑完全硬编码，不再读取字典
            # rule_sean_series = df_rules[(df_rules['商家(old)'] == "肖恩") & (df_rules['is_regex'] == 0)]
            # if not rule_sean_series.empty:
            ...
            df_transfer_in['对象'] = "" # (修复) 对象清空
            
            df_transfers = pd.concat([df_transfer_out, df_transfer_in])
            
            # (修复) 只 join 必需的列，不 join _source_tag
            df_transfers = df_transfers.join(df_income_sean[['交易时间']])
            
            # C. 填充通用列 (硬编码)
            df_transfers['项目'] = ""
            df_transfers['名称'] = ""
            df_transfers['描述'] = "" # (修复) 描述清空
            df_transfers['商家'] = "" # (修复) 商家清空
            df_transfers['标签'] = "" # (修复) 标签清空
