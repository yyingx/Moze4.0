...
DATA_SOURCE = {
    ...
    'Eggs': ["鸡蛋", "土鸡蛋", "生咸鸭蛋", "皮蛋"],
    'COOKED': [
        "熟食", "水煮花生", "毛豆", "藕夹", "茄盒", "锅包肉", "猪头肉", "卤菜", "凉菜", "烧鸡",
...
def process_transfers(df, main_col, sub_col):
    ...
    def add_pair(sub_df, out_a, in_a, sub_val='转账'):
        ...
        o['金额'] = pd.to_numeric(o['金额(元)']) * -1
        o['记录类型'] = '转出'
        o['账户'] = out_a
        o[sub_col] = sub_val

        i['金额'] = pd.to_numeric(i['金额(元)'])
        i['记录类型'] = '转入'
        i['账户'] = in_a
        i[sub_col] = sub_val
        res.extend([o, i])
...
def main():
    """主程序入口"""
    print(f"{BColors.BOLD}=== Moze 导入工具 v11.36 (代码规范化修复版) ==={BColors.ENDC}")
    try:
        ...
        if '标签' in df_final.columns:
            ...
            df_final.loc[m, '标签'] = (
                df_final.loc[m, '标签'].fillna(
                    "") + " " + df_final.loc[m, '_source_tag']
            ).str.strip()
        ...
        path = TARGET_DIR / \
            f"MOZE导入_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
