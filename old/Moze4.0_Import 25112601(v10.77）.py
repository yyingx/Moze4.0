...
DATA_SOURCE = {
    'FRUIT': [
        ...
        "桔子", "沙糖桔", "葡萄", "哈密瓜", "西瓜", "甜瓜", "柚子", "柠檬", "橙子", "菠萝", 
        "凤梨", "榴莲", "山竹", "蓝莓", "荔枝", "龙眼", "椰子", "柿子", "杨梅", "李子", 
        "芒果", "猕猴桃", "火龙果", "百香果", "莲雾", "车厘子", "牛油果", "鳄梨", "芭乐", 
        "番石榴", "菠萝蜜", "桑葚", "枇杷", "杨桃", "无花果", "圣女果", "小番茄", "姑娘果", 
        "西梅", "青枣", "冬枣", "黑布林", "人参果", "耙耙柑"
    ],
    'DRINK': [
        "可乐", "红牛", "奶茶", "东鹏", "饮料", "汽水", "果汁", "椰汁", "酸奶", "咖啡", "拿铁", "乐虎"
    ],
    'WATER': [
        "纯净水", "矿泉水", "农夫山泉", "怡宝", "百岁山", "娃哈哈", "今麦郎"
    ],
    'SOFTWARE': ["软件", "APP", "应用"],
    'CHARGING': [
    ...
    'VEGETABLE': [
        "青菜", "白菜", "娃娃菜", "菠菜", "生菜", "油麦菜", "韭菜", "芹菜", "香菜", "土豆", 
        "茄子", "豆角", "辣椒", "青椒", "洋葱", "西红柿", "番茄", "黄瓜", "冬瓜", "南瓜", 
        "苦瓜", "丝瓜", "萝卜", "胡萝卜", "藕", "莲藕", "笋", "莴笋", "蘑菇", "香菇", 
        "金针菇", "木耳", "豆芽", "玉米", "红薯", "山药", "芋头"
    ],
    'BEAN_PRODUCT': [
        "豆腐", "豆皮", "腐竹", "豆干", "豆腐泡", "素鸡", "千张", "豆卷", "腐皮", "油豆皮", 
        "内脂豆腐", "老豆腐", "嫩豆腐", "冻豆腐", "响铃卷"
    ],
    'INGREDIENTS': [
        "馒头", "火腿", "葱", "姜", "蒜", "酱料", "料酒", "白砂糖", "生抽", "老抽", 
        "蒸肉粉", "榨菜", "辣椒酱", "老干妈", "胡椒粉"
    ],
    'PORK': [
    ...
    'Raw chicken': [
        "三黄鸡", "鸡腿", "鸡翅", "鸡胸肉", "老母鸡", "乌骨鸡",
        "鸭肉", "鸭腿", "鸭翅", "鸭架", "老鸭"
    ],
    'BEEF_MUTTON': [
        "牛肉", "羊肉", "牛排", "肥牛", "羊排", "牛腩", "牛柳", "羊腿", "牛腱"
    ],
    'SEAFOOD': [
    ...
    # [核心] 熟食库 (必须包含鱿鱼圈)
    'COOKED': [
        "水煮花生", "毛豆", "藕夹", "茄盒", "锅包肉", "猪头肉", "卤菜", "凉菜", "熟食", 
    ...
    'MEAL': [
        ...
        "麻辣烫", "煎包", "煎饼", "烧饼", "锅盔", "馕", "土豆鸡蛋饼", "热干面", "牛肉面", 
        "刀削面", "盖浇饭", "炒饭"
    ],
    'DAILY_NECESSITIES': [
        "日常用品", "锅", "勺", "刀", "砧板", "擀面杖", "和面棒", "打蛋器", "压汁器", 
        "料理机", "计时器", "切蛋器", "瓶起子", "筷", "碗", "杯", "袋", "瓶", "盒", 
        "壶", "罐", "桶", "纸", "手套", "膜", "布", "洗洁精", "钢丝球", "牙刷", "牙膏", 
        "漱口杯", "洗发水", "沐浴露", "香皂", "鼻毛修剪器", "掏耳勺", "抽纸", "卫生纸", 
        "湿巾", "清凉油", "花露水", "洗衣粉", "洁厕灵", "除锈剂", "清洗剂", "拖把", "扫把", 
        "水桶", "盆", "围裙", "衣架", "晒衣杆", "夹子", "挂钩", "置物架", "凳子垫", 
        "雨伞", "锁", "灯", "电池", "插座", "理线器", "螺丝刀", "卷尺", "润滑油", "胶水"
    ],
    'BAD_SHOP': [
        "抖音", "支付宝", "微信", "转账", "扫码", "付款", "商户", "美团", "饿了么", 
        "合并支付", "京东", "拼多多", "淘特", "特价版"
    ]
}
...
def process_heuristics(df, main_col, sub_col):
    ...
    # 6. [逻辑铁腕] 大类识别 (熟食沉底，确保覆盖)
    mask_is_meal = check_keywords('MEAL')
    mask_is_cooked = check_keywords('COOKED') # [关键] 熟食锁
    ...
    ordered_checks = [
        ...
        ('egg', '鸡蛋', '食材'),
        ('INGREDIENTS', '', '食材'),
        ('COOKED', '熟食', '食材') # [关键] 熟食放最后，覆盖原材料
    ]
    
    for key, name, sub_c in ordered_checks:
        mask = check_keywords(key)
        
        # [关键修正] 
        # 如果是生鲜类 (海鲜/肉/菜)，必须排除掉 熟食(COOKED) 和 正餐(MEAL)
        # 这样 "炸带鱼" (既匹配SEAFOOD也匹配COOKED) 不会被识别为 "海鲜"，而是留给后面的 COOKED 识别
        if key in ['SEAFOOD', 'PORK', 'Raw chicken', 'BEEF_MUTTON', 'VEGETABLE']:
    ...
    mask = mask & (uncat | (df[main_col].isin(['购物', '居家', '饮食'])))
    if mask.any():
...
def process_main(df, df_rules, main_col, sub_col):
    ...
    # 高价值词库
    HIGH_VALUE_KEYS = ['FRUIT', 'VEGETABLE', 'BEAN_PRODUCT', 'INGREDIENTS', 'COOKED', 'MEAL', 'RICE', 'CHARGING', 'DAILY_NECESSITIES', 'PORK', 'Raw chicken', 'egg', 'BEEF_MUTTON', 'SEAFOOD']
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v10.77 (熟食优先级终极修复版) ==={bcolors.ENDC}")
    try:
