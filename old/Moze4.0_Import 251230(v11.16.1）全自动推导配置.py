...
# (记录类型, 主类, 项目)
RAW_MAPPING_CONFIG = {
    ('支出', '饮食', '食'): ['午餐', '夜宵', '早餐', '晚餐', '纯净水', '零食', '食材', '饮料水果', '熟食'], # [修改] 确保熟食也在项目'食'下
    ...
    try:
        ...
        if file_path.suffix.lower() == '.csv':
            try:
                ...
                print(f"  提示: 编码非 gb18030，切换 UTF-8 读取...")
                df = pd.read_csv(file_path, header=24, encoding='utf-8')
            ftype = "AliPay"
...
def process_main(df, df_rules, main_col, sub_col):
    df = df[(~df.get("当前状态","").isin(["已全额退款", "交易关闭"])) & (df.get("收/支") == "支出")].copy()
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v11.22 (增加熟食关键词) ==={bcolors.ENDC}")
    try:
        ...
        df_final.sort_values(['_Sort_Date', '记录类型'], ascending=[False, True], inplace=True)
