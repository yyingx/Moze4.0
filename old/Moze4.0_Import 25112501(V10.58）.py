...
def process_main(df, df_rules, main_col, sub_col):
    ...
    # 高价值词库 (去除了 FUEL)
    HIGH_VALUE_KEYS = ['FRUIT', 'VEGETABLE', 'BEAN_PRODUCT', 'INGREDIENTS', 'COOKED', 'MEAL', 'RICE', 'CHARGING', 'DAILY_NECESSITIES', 'PORK', 'Raw chicken', 'egg']
    ...
    if '备注' in df.columns:
        ...
        # 1. 覆盖：垃圾店铺 OR (备注有金子 AND 垃圾店铺? 还是所有情况?)
        # 修正回 V10.49 逻辑：只在垃圾店铺时覆盖，或者 备注有金子 且 是垃圾店铺？
        # 您的要求：垃圾店铺 -> 覆盖；正常店铺 -> 拼接
        
        # 这里的逻辑微调回 v10.43/49 的“垃圾店铺优先覆盖”原则
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v10.58 (燃料规则回滚版) ==={bcolors.ENDC}")
    try:
