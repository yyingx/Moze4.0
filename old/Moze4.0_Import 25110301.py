...
def sniff_and_load_data(file_path: Path):
    ...
    if file_type is None:
        ...
        print("请确保选择 原始微信账单 / 原始支付宝账单 / 合并后的账单.xlsx", flush=True)
        if excel_workbook_object: excel_workbook_object.close()
        return None
...
def process_transactions(df_source, df_rules, start_datetime_obj):
    ...
    refund_condition = (df_source["当前状态"] != "已全额退款") if "当前状态" in df_source.columns else True
    df_expenses = df_source[expenses_condition & refund_condition].copy()
    print(f"已筛选出 {len(df_expenses)} 条 '支出' (且未退款) 记录进行处理...", flush=True)
    ...
    # 创建 df_target (用于存放最终数据)
    df_target = df_expenses.copy()
    
    # a. 准备合并：将交易对方与规则的 '商家(old)' 匹配
    df_target['商家(old)'] = df_target['交易对方'].str.strip()
    
    # b. 核心：使用 left merge 将规则应用到所有行
    df_target = pd.merge(
        df_target, 
        df_rules, 
        on='商家(old)', 
        how='left',
        suffixes=('', '_rule') # 原始列不变, 规则列加后缀
    )
    ...
    # d. (硬编码) 处理充电桩特殊逻辑 (向量化)
    print("...正在应用充电桩特殊逻辑...", flush=True)
    is_charging = df_target['商品'].str.contains("自助服务-充电桩", na=False)
    
    # 一次性赋值
    df_target.loc[is_charging, ['名称', '主类别', '子类别']] = ['充电', '交通', '加油充电']
    
    # 处理充电桩的描述 (split)
    charging_desc_split = df_target.loc[is_charging, '商品'].str.split('/', n=1, expand=True)
    if not charging_desc_split.empty:
        # 仅当 split 成功且第二部分 (index 1) 不为空时才使用
        valid_desc = charging_desc_split[1].str.strip()
        # .where 条件为 True 时保留 valid_desc, 否则使用原 '商品'
        df_target.loc[is_charging, '描述'] = valid_desc.where(
            valid_desc.notna() & (valid_desc != ''), 
            df_target.loc[is_charging, '商品']
        )
