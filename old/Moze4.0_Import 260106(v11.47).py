"""
Moze 导入脚本 v11.47 (Final Restoration)
Created on Sun Jan 05 2026
Fixed: Tue Jan 06 2026 (Restored 'apply' for dates to prevent data loss)
@author: TZY_YX
"""
...
#     """
#     [v11.47 关键修复] 使用 apply 逐个处理，确保原始数据不被覆盖
#     """
...
#     # 3.1 尝试标准转换
#     try:
#         dt = pd.to_datetime(s_x, errors='raise')  # 先尝试严格转换
#         return dt
#     except:
#         pass

#     # 3.2 尝试作为数字字符串处理 (例如 "45234")
#     try:
#         num = float(s_x)
    ...
    try:
        if file_path.suffix.lower() == '.csv':
            for enc in ['gb18030', 'utf-8']:
                try:
                    preview = pd.read_csv(file_path, header=None, nrows=50, encoding=enc, names=list(
                        range(30)), engine='python')
...
def main():
    print(f"{BColors.BOLD}=== Moze 导入脚本 v11.47 (Final Restoration) ==={BColors.ENDC}")
    try:
