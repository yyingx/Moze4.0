...
def process_heuristics(df, main_col, sub_col):
    ...
    # 9. 软件名称智能提取 (保留此项，因为不涉及备注清洗，只涉及提取)
...
def process_main(df, df_rules, main_col, sub_col):
    ...
    # [最终去格式] 仅去掉首尾空格和换行符，保留所有内部字符(数字、字母、特殊符号)
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v11.04 (极简瘦身版) ==={bcolors.ENDC}")
    try:
