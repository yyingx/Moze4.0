...
def process_heuristics(df, main_col, sub_col):
    ...
    # 如果是 "土豆鸡蛋饼"，check_keywords(MEAL) 为真，则不匹配蔬菜/大米/豆制品/熟食
    mask_is_meal = check_keywords(PATTERNS['MEAL'])
    
    ordered_checks = [('VEGETABLE', '蔬菜', '食材'), ('RICE', '大米', '食材'), ('BEAN_PRODUCT', '豆制品', '食材'), ('COOKED', '熟食', '食材')]
    ...
    mask = mask & (~mask_is_meal) & (~mask_already_set) & (uncat | (df[main_col].isin(['购物', '居家', '饮食'])))
    if mask.any():
...
def process_main(df, df_rules, main_col, sub_col):
    ...
    is_meal = (
        ...
        df['描述'].str.contains(PATTERNS['MEAL'], regex=True, na=False) # [关键] 增加描述列扫描
    ) & uncat_mask
...
def main():
    print(f"{bcolors.BOLD}=== Moze 导入工具 v10.37 (双重逻辑锁修正版) ==={bcolors.ENDC}")
    try:
