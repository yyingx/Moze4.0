...
class MozeOCRTool:
    def __init__(self):
        print(f"{BColors.OKGREEN}🚀 Moze 4.0 终极版 (无描述) 启动...{BColors.ENDC}")
        try:
                        ...
                        for i in range(len(line_items)-1, -1, -1):
                            ...
                            if '.' in clean and len(clean) > 2:
                                try:
                                    ...
                                    if (0 <= val <= 24.59) and not any(s in txt for s in ['+','-','¥']): continue
    ...
    def save(self):
        ...
        for i, r in zeros.iterrows():
            m = df[(df['日期']==r['日期']) & (abs(df['金额'])==abs(r['金额'])) & (df['时间']!='00:00:00')]
            if not m.empty: df.at[i, '时间'] = m.iloc[0]['时间']
        ts = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        fp = EXPORT_DIR / f"MOZE_Filter_NoDesc_{ts}.csv"
