# -*- coding: utf-8 -*-
"""
Moze 导入脚本 v11.67 (Code Optimization + Feature Enhancement)
...
CHANGELOG (v11.67):
[新增] 点号分隔格式 → 备注"借入谢辉.钉子"自动解析为 对象=谢辉, 描述=钉子
[新增] 自动推断收/支 → 手动添加行时可不填"收/支"列，系统根据备注关键词自动判断
[新增] 报销关键词识别 → 材料费/燃油费/物流运输等开头的备注自动识别为报账

OPTIMIZATION (v11.67):
[优化] 删除空函数 clean_final_description
[优化] 提取重复常量 DEBT_KEYWORDS, REIM_TRAVEL_KEYS, REIM_EXPENSE_KEYS
[优化] 移除硬编码绝对路径
[优化] 合并重复的条件判断
[优化] 循环改向量化操作，提升性能
[优化] 拆分 process_main 为多个小函数
[优化] 统一备注解析逻辑到 parse_memo_subcategory 函数
"""
...
GENERIC_PATTERN = re.compile(rf"^({'|'.join(map(re.escape, ALL_GENERIC_KEYS))})(.*)")
...
def process_main(df_in, df_rules, main_col, sub_col):
    ...
    # [NEW v11.67] 自动推断空白的收/支列
    # 当手动添加借入/借出/报销记录时，可不填"收/支"列，系统根据备注关键词自动判断
...
def main():
    print(f"{BColors.BOLD}=== Moze 导入脚本 v11.67 (Optimized) ==={BColors.ENDC}")
    try:
