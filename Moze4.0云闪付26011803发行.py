            ...
            for img_slice in slices:
                ...
                for line_items in lines:
                    ...
                    if not is_bank_row:
                        for i in range(len(line_items) - 1, -1, -1):
                            ...
                            if '.' in clean and len(clean) > 2:
                                ...
                                final_clean = final_clean.replace('l', '1').replace('I', '1').replace('|', '1').replace('[', '1').replace(']', '1')
                    ...
                    if (has_amount or has_keyword) and not is_bank_row:
                        ...
                        elif "收到" in clean_text: target_name = clean_text.split("收到")[1]
                        else:
                            temp = full_text
                        ...
                        target_name = target_name.replace("半", "").replace("收入", "").replace("支出", "").replace("一", "").replace("Y", "").replace("¥", "")
                        target_name = target_name.replace("壬", "王").replace("玍", "王").replace("酃", "").replace("鬣", "")
                        target_name = re.sub(r'[^\u4e00-\u9fa5a-zA-Z]', '', target_name).strip()
    ...
    def verify_loops(self):
        ...
        for idx, row in transfers.iterrows():
            ...
            if not potential_partners.empty:
                ...
                t2 = partner_row['时间']
                best_time = t1
                if t1 == "00:00:00" and t2 != "00:00:00": best_time = t2
                elif t2 == "00:00:00" and t1 != "00:00:00": best_time = t1
                
                # 检查并修正
                if self.rows[idx]['时间'] != best_time:
                    print(f"      🔧 [时间同步] {current_date} ({current_amount}): {self.rows[idx]['时间']} -> {best_time}")
                    self.rows[idx]['时间'] = best_time
                    fixed_time_count += 1
                    
                if self.rows[partner_idx]['时间'] != best_time:
                    print(f"      🔧 [时间同步] {current_date} ({-current_amount}): {self.rows[partner_idx]['时间']} -> {best_time}")
                ...
                success_pairs += 1
                 ...
                 fail_list.append(f"{current_date} {row['时间']} ({current_amount})")
        ...
        if not fail_list:
            print(f"   🎉 {BColors.OKGREEN}完美！{success_pairs} 对转账全部闭环成功。{BColors.ENDC}")
        else:
            print(f"   ⚠️ {BColors.WARNING}警告: 发现 {len(fail_list)} 笔孤立转账，请核对:{BColors.ENDC}")
